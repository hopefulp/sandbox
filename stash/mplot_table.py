...
def draw_table(inf,Llevel,icx,icy,job,title,xlabel,ylabel,legends,Lsave,yscale,colors,Ltwinx,icy_right,ylabel_r):
    '''
    inf : table format
    ...
    for i in icy:
        ...
        if not legends:
            legends.append(ylegend[i])
        
    print(f"title = {title} xlabel = {xlabel}")
    print(f"x {x}, yplot {yplot}, legend {legends}")
    ### x, yplot, y_legend are lists
    ### x will be used for just len(x)
    if Llevel:
        mplot_levels(x, yplot, title=title, xlabel=xlabel, ylabel=ylabel, legend=legends,colors=colors)
    else:
        mplot_2d(x, yplot, title=title, xlabel=xlabel, ylabel=ylabel, legend=legends,colors=colors)
    return 0
...
def main():
    ...
    parser.add_argument('-t', '--title', default='Vacancy Formation Energy', help='title of figure would be filename')
