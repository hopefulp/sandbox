...
def common_figure(ctype='dark', colors=None, Ltwinx=False):
    '''
    ctype   darken to change intensity
            cycle to use designated color turns
    '''
    print(f"colors: {colors} in {whereami()}")
    if ctype == 'darken':
...
def mplot_nvector(x, y, dx=1.0, title=None, xlabel=None, ylabel=None, legend=None,Lsave=False, colors=None):
    ...
    y:: [size] or [[size],[size],...[size]]
    '''
    print(f"mplot_nvector:: {legend} {len(legend)} {whereami()}")
    nlegend = len(legend)
    if colors:
        #fig, ax = common_figure(colors=colors)
        fig, ax = common_figure()
    else:
        fig, ax = common_figure(colors = nlegend)
    #else:
    ...
    if ys.ndim == 1:
        ...
        for i in range(len(ys)):
            ...
            plt.plot(x,ys[i,:], colors[i], label=legend[i], markersize=10 )
