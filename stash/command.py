...
def show_command(work, subwork, job_submit, qname_in, vjob, inf, keyvalues, nodename, nnode, nproc, nodelist, sftype, dtype, partition,poscar, nhl,idata,ndata):

    ### -s for POSCAR or CONTCAR
    if '.' in qname_in:
        qname=f_ext(qname_in)
    else:
        qname=qname_in
    ...
    kisti.vas += f"\n\t    :: SLAB XY-relax"
    kisti.vas += f"\n\t\t$ kpy vas_make_ini.py -s {qname_in} -j copt -exe xyrelax"
    ...
    kisti.vas += f"\n\t    :: FAKER Job & OVERwrite"
    kisti.vas += f"\n\t\t$ kpy vas_make_ini.py -j fake -s {qname_in} -sj {vjob} -al -ra -d d{datetime.now().strftime('%d%H')} -n 6 -e g : more info_vasp.py"
    kisti.vas += f"\n\t\t$ kpy vas_make_ini.py -j {vjob} -r on -s {qname_in} -d dnameid : o-overwrite n-not submit job"
    kisti.vas += f"\n\t    :: (OORINano)"
...
def main():
    ...
    show_command(args.work,args.subwork,args.job_submit,args.qname,args.vas_job,infile,args.keyvalues,args.nodename,args.nnode,args.nproc,args.nodelist, args.func_type,args.data_type,args.xpartition,args.poscar, args.hidden_layers, args.idata, args.ndata)
