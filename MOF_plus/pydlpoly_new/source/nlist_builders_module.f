...
cRS   add a feature to bypass the neigborlist recomputation test (for optimization)
cRS      and an option to enforce it
      ...
      save nlst_notest, nlst_rebuild, nlst_report

      data nlst_notest/.false./,nlst_rebuild/.false./,
     x     nlst_report/.false./
...
c*********************************************************************
      ...
      integer natms,idnode,mxnode,imcon,nneut,keyfce,i
...
cRS     only for MD (not working for optimization of velocities are zero => set nlst_notest to true
        if (nlst_notest) then

          if (nlst_rebuild) newlst = .true.
...
ccs
cRS  what is this for??? chklist is always true it seems
cRS        if(.not.chklist) newlst = .true.
ccs
...
c     ignore real space distant interactions
     ...
     x          (newlst,natms,idnode,mxnode,imcon,rcut,delr)
            ...
            if(.not.loglnk)then 
              if (nlst_report) write (*,*) "DEBUG!!  parneulst called"                            
              call parneulst
     x          (newlst,lneut,nneut,idnode,mxnode,imcon,rcut,
     x          delr)
              
            else
              if (nlst_report) write (*,*) "DEBUG!!  parlinkneu called"                            
              call parlinkneu
