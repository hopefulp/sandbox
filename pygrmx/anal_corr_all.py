...
from _mplot2d import _mplot_2c

def alldir(d_suff,d_suff2,d_pre,fname,e_limit,Lexe):
    p_dir = os.getcwd()
    lists   = os.listdir('.')
    nfile   = 0
    l_ene   = []
    ...
    _mplot_2c(l_pico, corr_t,Lsave,fig_name,title,xt,yt)
...
def main():
    ...
    parser.add_argument('-s1', '--dsuf1', help='suffix of directory in search')
    parser.add_argument('-s2', '--dsuf2', help='suffix of directory in search')
    ...
    alldir(args.dsuf1,args.dsuf2,args.dpre,args.enefile,args.elimit,args.run)
    return 0
