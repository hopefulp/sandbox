...
def run_job(job, ifile, icharge, mdfile, dyn, jobname, grofile):
    ...
    ### Step 4: check .mol2-charge then go to tleap
        ...
        os.system(com)
        q1 = "is it OK (never mind, it will be fixed at top file) ?"
        if common.yes_or_no(q1):
            ...
            print("just type \"yes\": charge will be fixed later on gromacs .top file")
        return 0
...
def main():
    ...
    parser.add_argument('work', choices=['mol2pdb','pdb2mol2','frcmod','tleap','amber2grmx','topchg','waterbox','modtop_water','pre_pp','modtop_ion','do_pp','submit'], help='individual jobs from qchem input file to gromacs MD')
    ...
    parser.add_argument('-f', '--mdpfile', help="MD Parameter file, used for grompp")
    parser.add_argument('-d', '--dynamics', default='md', choices=['md', 'min'], help="dynamics style, used for .tpr filename")
    parser.add_argument('-j', '--jobname', help="input jobname of qsub, used in \"submit\" for dir_name")
    parser.add_argument('-u', '--usage', action='store_true', help=( "Usage of each work :: %s  work-type -u" % os.path.basename(__file__)))
    parser.add_argument('-g', '--grofile', help="gro file used for grompp: instead of _sol.gro, _sol_neut.gro")
    ...
    if args.usage:
        ...
        elif args.work == 'tleap':
            print("%s %s -i a.mol2" %  (os.path.basename(__file__), args.work))
            print("just check charge of a.mol2, will advance to tleap")
