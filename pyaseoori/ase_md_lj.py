...
def md_run(atoms, count=100, T=300, trajname="emtmd.traj"):
    ...
    dyn = NPT(atoms, timestep=2*units.fs, temperature_K = T * units.kB, externalstress = 4.3e-7, \
        ttime=25*units.fs, pfactor=9.83e-6) # at P_t, 9.83e-4
...
def aux_md(inf, job, dyn):
    if inf:
        ...
        ### use 256 Ar atoms
        atoms = build_structure('bulk', status='gas', natom=256, latt=23)
