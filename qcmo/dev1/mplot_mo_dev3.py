...
################### MAIN LOOP ####################
def analyze_files(files,atomlists, motypes, base_type,L_link): 
...
# func
    # for file
        # while
            ...
            #if L_mo_coeff: # and tag_moc=="YES": # "cut" means alpha has done
                #### Look for KW_MO coeff
            if flag_moc == "OFF" and tag_moc=="YES" :
                ### first block for MOC: find key_word_MOcoeff and initialize
            ...
            ############ if-flag_moc==ON block
                ...
                ###### analyze ONE MOC line for one basis in MOC_BLOCK
                    ...
                    #### for each MOC line, SAVE Nene_mocline==6 (if last, Nene_mocline<=6) in moc_block
                    for j in range(Nene_mocline): # for each imoc line of 1 atomic basis
                        mo_symbol=[]
                        moc_tmp=[]
                    ...
                    if imoc == 1+nbasis:
                        ...
                        if V_print_moc >= 1: 
                            print "%d-th moc_block" % imoc_block
                            if imoc_block == 19:
                                for ele in sorted_moc_block:
                                    print ele
                        for eff_coeff in sorted_moc_block: # to the end
                            ...
                            if int(eff_coeff[1]) <= ncore:            # NCORE is treated here
                                ...
                                if V_print_log >=1: print "last: ", f_list_imo_sel_ab
                                imo_select=f_imo_dump(dump_mo_id_block, Fl_MO_atoms[fid], Fl_MO_types[fid],imo_tmp)
                                if imo_select:
                                    f_list_imo_sel_ab.append(imo_tmp)
                            ...
                            elif Fl_MO_types[fid] == "SUB":    # 'all' for the selected atom 
                                f_tag=0
                            ...
                            #### if index MO is changed, dump
                                ...
                                #### dump for 1-imo, the previous saved moc's
                                #### FILTER 4.5 :: the imo-block will be dumped?
                                #### imo_select returns 1 if dump or 0 for skip
                                #print dump_mo_id_block, whereami()
                                imo_select=f_imo_dump(dump_mo_id_block, Fl_MO_atoms[fid], Fl_MO_types[fid], imo_tmp)
                                #### RECORD and INITIALIZE
                                #print  imo_tmp, mo_ene_tmp, imo_sel
...
def main():
    ...
    analyze_files(args.outfiles, args.atomlists, args.motypes , args.basetype, args.link)
    return(0)
