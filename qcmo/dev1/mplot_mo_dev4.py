...
################### MAIN LOOP ####################
def analyze_files(files,atomlists, motypes, base_type,L_link, link_types, atom_groups): 
                    ...
                    if imoc == 1+nbasis:
                        ...
                        for eff_coeff in sorted_moc_block: # to the end
                            ...
                            ### FILTER 3: cut lines using FL_nlumo[0] 
                                ...
                                imo_select,mbasis,mcoeff=f_imo_dump(imoc_valuable, Fl_MO_atoms[fid], Fl_MO_types[fid],imo_tmp)
                                if imo_select:
                                    f_list_imo_sel_ab.append(imo_tmp)
                                    f_list_ene_sel_ab.append(mo_ene_tmp)
                                    a = QC_IMO(imo_tmp, mbasis, mcoeff, mo_ene_tmp)
                                    fl_QCIMO.append(a)
...
# func
    ...
    if L_link:
        ...
        for i in range(nfile-1): 
            ...
            elif link_types[i] == 'split':
                if side == 'l':
                    fl_molink_hl_id=get_link(Fl_QCIMO[i+1],ihomo_f1,Fl_QCIMO[i],ihomo_f0,'split', nlink, atom_groups)
                elif side == 'r':
...
def main():
    ...
    parser.add_argument("-lt", "--linktypes", nargs='+', default=['flow', 'split'], help="for the same molecule (flow) and combination with other molecule (split)")
    parser.add_argument("-sg", "--splitgroup", nargs="+", help="divide atom groups into two group for split link")
    args=parser.parse_args()
    analyze_files(args.outfiles, args.atomlists, args.motypes , args.basetype, args.link, args.linktypes, args.splitgroup)
