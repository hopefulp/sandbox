    ...
    # if L_link: 
        ...
        #for n in range(nfile-1):
            ...
            ### To treat degenerated molecule
                ...
                if (not L_degen_f[0]) and (not L_degen_f[1]):
                    print("Only no degenracy works")
                    x=Xrange_nf(nfile, i, Fl_beta_tag[i], f0ab, 1)
                    xl=x[1]                 # 1=right side of 0-th file is left-link
                #### if CO2.out: use degeneracy=2 
                #fname_list= Fl_qchem_outf[i+1].split('.')
                #if vp_draw >=1: print(f"Drawing link of RHS of {fname_list[0]}")
                #    x=Xrange_nf(nfile, i+1, , 0, 2)
                #    if vp_draw >=1: print(f"x in degen=2: {x}")
                #    xr1, xr2 = x[0], x[2]
                #else:
