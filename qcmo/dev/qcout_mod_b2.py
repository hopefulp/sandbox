...
def extract_ene_4line(index_homo_line, line_ene_ab):
    ...
    print(f"In {whereami()}(): HOMO energy, ID, beta \n\t\t{e_homo_ab}  {e_homo_id_ab}  {beta}  ")
...
def obtain_MOC(f,beta,homo_id_ab,norb_ab,atomlist=None,mo_type=None):
    """
    ...
    print(whereami(), " :: atomlist: {} mo_type: {}".format(atomlist, mo_type))
    flag_block = "OFF"
    flag_moc_beta = "OFF"
    ...
    for line in f.readlines():
        ...
        if flag_block == "OFF":
            if re.search(KW_MOcoeff, line) :    #common for alpha & beta
                if vprint_moc >=1: print("MOC found: start w. initialization")
                flag_block="YES"
                #### constants from module qcout_ini
                max_nlumo = Nmax_virtual
                ...
                ihomo_a = homo_id_ab[0] # Alpha has always large value
                ilumo_cut=ihomo_a+max_nlumo
                #lumo_id_cut=f_homo_id+Fl_nlumo[fid]
                ### define lumo index cut
                if norb_ab[0] < ilumo_cut:
                    ilumo_cut = norb_ab[0]
                print ("HOMO id & Virtual Orbital cut id ", ihomo_a, ilumo_cut)
                if flag_moc_beta == "ON":
                    f_homo_id=FL_homo_id[fid][1]
                    print ("###################  BLOCK 2 :: MO Coeff beta starts ")
                    k=0
                    i_block=0
                    p_imo=0            # mo-id starts from 1 in MO Coefficient Block
                    p_moe=-100
                    imoc_valuable=[]
                    l_imo_sel_ab=[]
                    l_ene_sel_ab=[]
        ...
        ######################################################### INSIDE MOC PART
            ...
            ######### BLOCK inside analyze
            ###### escape if end of MO Coefficients
            if vprint_loop>=1: print(f"k increase {k}")                       # this is OK
            if not line.strip():
                if beta == 0:
                    print ("End of MOC of alpha")
                    break
                else:
                    print("MOC beta is not constructed")
                    sys.exit(2)
            #### First line in MOC Block: save MO id 
            if k == 0:                                       # k runs 0 - nbasis+1, inside MOC 1 block
                ...
                #print(l_imo)
                k += 1
                ### Block initializatio here
                p_imo=int(l_imo[0])
                imoc_valuable=[]
                continue
            #### Second line: save energy
            elif k == 1:
                ...
                continue
            # if's column
            ###### analyze ONE moc line for one basis in a MOC_BLOCK: from 2 to 1+nbasis line
            elif k <= 1+nbasis:                                      # from k==2, nbasis starts
                # moc_line_analysis(line, Ne_col_block, temp_atom_name)
                lmoc_line=line.split()
                if vprint_moc >=2 and k==2: print ("moc-i {}".format(i-1), lmoc_line)
                ...
                ########  Each MOC line is decomposed to maximum Ne_col_block==6 basis-ene-coeff data in moc_block[]
            ...
            # if inside block: k <= 1+nbasis
                ####### After reading one MOC block (6 mo_id), Treat it and INITIALIZE variables again
                if k == 1+nbasis:
            ...
            # if inside block: k <= 1+nbasis
                # if k == 1+nbasis
                    ...
                    #### scan sorted sorted moc_block[] of moc-format
                    for moc_block_ele in sorted_moc_block:      # to the end
                        ...
                        ### when b_imo > ncore: escape to here
                        ...
                        if b_imo > ilumo_cut:
                            ...
                            #if vprint_log >=1: print ("last: ", l_imo_sel_ab)
                            #imo_select,bas_co_dic=imo_basis(imoc_valuable, input_atoms_id, mo_type,p_imo), not needed any more
                            ####imo_select=f_imo_dump(imoc_valuable, atoms, mo_type,p_imo)
                            #if imo_select:
                            #    n_loop += 1
            ...
            # if inside block: k <= 1+nbasis
                # if k == 1+nbasis
                    ...
                    #a = QC_IMO(p_imo, atom_basis, p_moe)
                    ### initialize moc variable for new block: these cannot be used here
                    k=0                         # k counts one block: 0 ~ nbasis+1
                    i_block+=1
                    moc_block=[]
                    continue    # if k == 1+nbasis: set k=0, do not raise by next line
...
def imo_basis(imoc_list, l_atoms, filter_tag, mo_id):
    ...
    if not imoc_list:
        ...
        if filter_tag=="ONE" :
            if atom_tag[0]==1:
                base_coeff = imo_basis_dic(imoc_list)
                selection= 1
            else: 
                selection= 0
    ...
    if vprint_moc >=1: 
        print (f"mo_id {mo_id} in {whereami()}()")
...
def imo_basis_dic(imoc_list):
    i=0
    bcoeff={}
    for line in imoc_list:
        if i == 0:
            if vprint_moc >= 2: print (line)
        else:
            if vprint_moc >= 2: print (line)
            b_coeff = line.split()
    ...
    if vprint_moc >= 2: print ("bases_coeff dictionary:", bcoeff  , whereami())
    return bcoeff
