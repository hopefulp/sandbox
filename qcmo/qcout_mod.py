    ...
    for line in f.readlines():
        ...
        if flag_block == "OFF":
            if re.search(KW_MOene_occ, line):
                print ("####################  MODULE 1 (MOE):: obtain MO Energy in {}:{}()".format(os.path.basename(__file__), whereami()))
                flag_block="ON"
...
def obtain_MOC(f,beta,homo_id_ab,norb_ab,atomlist_id=None,mo_type=None):
    ...
    """
    global n_loop
    print (f"####################  MODULE 2 (MOC) :: MO Coeff starts in {whereami()}() ")
    ...
    for line in f.readlines():
        ...
        if flag_block == "OFF":
            ...
            if re.search(KW_MOcoeff, line) :    #common for alpha & beta
                if vp_moc >=1: print("MOC found w. beta_tag \"{tag_beta}\": start w. initialization")
                flag_block="YES"
                ...
                if vp_moc >=1: print(f"atom_style is '{atom_style}' for atomlist_id {atomlist_id}")
                ...
                if tag_beta == "ON":
                    ...
                    print ("###################  MODULE 2 :: MO Coeff beta starts ")
                    k=0
                    i_block=0
                    imoc_valuable=[]
                    moc_block=[]
        ...
        #################### INSIDE MOC PART:: including all the MOC blocks
            ...
            ######### Deal With MOC PART: as unit of MOC MODULE (nbasis+2 lines)
            ###### escape if end of MO Coefficients w. Savind data
            ...
            if not line.strip():
                ...
                if beta == 0:
                    ...
                    print(f"Check-End of MOC BLOCK but not have read beta w. tag_beta {tag_beta}")
                    if tag_beta=='ON':
                        break
                    else:
            ...
            if k == 0:                                       # k runs 0 - nbasis+1, inside MOC 1 block
                ...
                if vp_moc >=3 : print ("k==0; imo =", line.strip())
            ...
            ##### save energy
                ...
                if vp_moc >= 3: print ("k==1; imo_ene", imo_ene)
            ...
            #### Analize ONE moc line for one basis in a MOC_BLOCK: from 2 to 1+nbasis line
                ...
                if vp_moc >=3 and k==2: print ("moc-i {}".format(k-1), lmoc_line)
                ...
                #if vp_moc >= 1: print ind_basis, l_symbol, lcoeff, atom_name, id_atom
                    ...
                    # for-loop in sorted moc_block[]
                        ...
                        ### when i_sortb == len(sorted_moc_block): last element is read, treat last b_imo in 6 imos
                            ...
                            if imo_select:
                                ...
                                a = QC_IMO(p_imo, p_moe, base_coeff_dict)
                                l_QCIMO.append(a)
                                if vp_moc >=1: print(f" {p_imo} imo:: base_coeff_dict {base_coeff_dict}")
...
def imo_dic_basis_coeff(imoc_list):
    ...
    for line in imoc_list:
        #if i == 0:
        #    if vp_moc >= 1: print (f"imoc list:{line}:{whereami()}()")
        #else:
        if vp_moc >= 4: print (f"imoc list:{line}:{whereami()}()")
...
### keywords for Natral Bond Orbital Analysis
...
KW_nao_chg="Summary of Natural Population"
...
def analyze_nbo(f, nao_chg, chg_sum_group):
    ...
    atomsum=""
    tag_write_sum="OFF"
    if vp_nbo >=1: print(f"flag_chg :: {flag_chg}, tag_chg :: {tag_chg}, atoms {nao_chg} group for sum {chg_sum_group}")
    i=0
    for line in f.readlines():
        ##### Charge at Summary of Natural Population
        if flag_chg:
            if not tag_chg :
                if re.search(KW_nao_chg, line):
                    tag_chg = "ON"
                    print(f"found KW NAO total charge: {KW_nao_chg}")
                    continue
                else:
                    i+=1
                    continue
                ...
                #### if end of block
                if re.search("===========", line):
                    if tag_write_sum != "Done":
                        print(f"SUM for {atomsum}: {chgsum} in {whereami()}()")
                    print("########## End of NAO Charge Block")
                    flag_chg = False            # finish flag_chg block
                    continue
                lline=line.strip().split()
                if len(lline) < 2 or not lline[1].isdigit():
                    continue
                else:
                    atom_name=lline[0]
                    atom_index=lline[1]
                    atom_chg=float(lline[2])
                    #atom_id=atom_name + atom_index
                    ...
                    if atom_name in nao_chg:
                        ...
                        if atom_name in chg_sum_group:
                            ...
                            chgsum+=atom_chg
                            atomsum+=atom_name
                            if not chg_sum_group:
                                print(f"SUM for {atomsum}: {chgsum:10.4f} in {whereami()}()")
                                tag_write_sum="Done"
                        if not nao_chg:
                            flag_chg = False    # finish flag_chg block
                    continue

        if tag_nbo == "YES" and flag_nbo == "NO" :
