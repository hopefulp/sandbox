...
#set EXE = "$sandbox/pyamp/amp_run_v2.py"
...
if ( ! $?scan ) then
    if ( $?dtype ) then
        if ( $?tef ) then
            if ( $?des ) then
                $PYTHON $EXE -f $fname -j $pyjob -tef -hl $hl -el $el -fl $fl -nc $nc -nt $nt -ntr $ntr -dtype $dtype -dl $dlist -des $des -pf $pf -pmm $pmm -pn $pn -g 
            else
