...
### variable meaning
#   f : for 1 file
#   _2d : for alpha, beta, last (last-1) dimension is 2
...
################### MAIN LOOP ####################
def analyze_files(files, L_link, atoms, i_mo_types, beta_option): 
    # start analyze_files function 
    print files, "L_link = ", L_link, "atom selection option = ", atoms, "mo types = ", i_mo_types
    """
        some molecule has beta as the same as alpha though it is restriced such as 1-PP-B-CO2
        so disregard beta for drawing
    ...
    if not beta_option:
        beta_option=[]
        while len(beta_option) < len(files):
            beta_option.append(0)
    #### analysis option from draw_func_anal_dev2
    nfile=len(files)
    #### if atom input for selective atom
    if not atoms:
    ...
    print Fl_MO_atoms, Fl_MO_type, Fl_nlumo, MO_link_hl_id
    #### get parameters from mplt_mo_ini : qchem_out files, nfile, nlumos, link_id, link option
    Fl_qchem_outf = files[:]
        ...
        # while
            ...
            #### have found MO
                ...
                #### 4. find blank for beta
                    ...
                    continue
                #### 5. find BETA : if beta_option == 0, ignore beta due to same energy
                elif re.search(KW_MOene_beta, line):
                    if not beta_option[fid] == '0':
                        tag_beta="YES"
...
def main():
    ...
    parser.add_argument("-a", "--atoms", help="2D atom list for selective MO drawing")
    ...
    parser.add_argument("-beta", "--betaspin", nargs="+", help="input beta option for bent CO2 to remove beta") 
    ...
    analyze_files(args.outfiles, args.link, args.atoms, args.motypes, args.betaspin)
    return(0)
