...
sub saveFile {
	...
	for $j (0 .. $#{ $data->{$i}{VALS} }) {
	    $rl = $j*$delR;
	    $ru = $rl + $delR;
	    $norm = $data->{$i}{CONST} * $tot * ($ru*$ru*$ru - $rl*$rl*$rl);
	    $data->{$i}{VALS}[$j] /= $norm;
...
sub calcRDF {
    ...
    my ($bin, $dist, $nBins, $count, $prec, $rounding, $soluMols);
    my ($GRID, $currCell, $numNeigh, $x, $y, $z, $nCells);
    my ($comATOMS, $solvAtoms, $soluAtoms, $density);
    ...
    if ($reImage) {
        @tmp = ("X", "Y", "Z");
        for $j (@tmp) {
            $BBOX->{$j . "COORD"}{CENTER} = $BBOX->{$j}{CENTER} = $BBOX->{$j . "COORD"}{len}/2;
            $BBOX->{$j . "COORD"}{hi} = $BBOX->{$j}{hi} = $BBOX->{$j . "COORD"}{len};
            $BBOX->{$j . "COORD"}{lo} = $BBOX->{$j}{lo} = 0;
            for $i (keys %{ $ATOMS }) {
    ...
    ($comATOMS, $soluAtoms, $solvAtoms) = getSoluSolvCOM($ATOMS, $SOLUTE, $SOLVENT);
    ($GRID, undef, undef) = CreateGrid($comATOMS, 0, $BBOX, $gridLen, 0);

    $count = 0;
    for $j (keys %{ $soluAtoms }) {
	$MOL = $soluAtoms->{$j};
	next if (! exists($MOL->{CELL}));
	($x, $y, $z) = ($MOL->{CELL}{XINDEX}, $MOL->{CELL}{YINDEX}, $MOL->{CELL}{ZINDEX});
	$currCell = $GRID->{$x}{$y}{$z};
	next if ($currCell->{VISITED});
	$currCell->{VISITED} = 1;
	$numNeigh = sprintf("%.0f", $rMax/$gridLen); # upper limit on number of neighbouring cells to search
	$nCells = GetNeighbours($GRID, $currCell, $numNeigh); # get all the neighbouring cells
	($soluMols, $solvMols) = getSoluSolv($ATOMS, $currCell, $nCells);
	for $i (@{ $soluMols }) {
	    for $k (@{ $solvMols }) {
		$dist = sprintf("%${prec}f",GetBondLength($i,$MOL));
		next if ($dist > $rMax or $dist == 0);
		$count++;
		$bin = sprintf("%${rounding}f",($dist/$delR) + 1);
		$DATA->{$frameNum}{VALS}[$bin]++;
	    }
	    delete $comATOMS->{$i->{INDEX}};
	}
    }

    $DATA->{$frameNum}{TOTAL} = $count;
    $DATA->{$frameNum}{CONST} =  4.0*PI/3;
    $density = $count/($BBOX->{XCOORD}{len}*$BBOX->{YCOORD}{len}*$BBOX->{ZCOORD}{len});
    $DATA->{$frameNum}{CONST} *= $density * $count;
}

sub getSoluSolvCOM {
    my ($allAtoms, $soluMols, $solvMols) = @_;
    my ($i, $count, %atomCOMS, %solu, %solv, $MOL);
