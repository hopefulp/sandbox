...
sub createCeriusFF {
    my ($data) = $_[0];
    my (%FF, $i, $atom, $j, @atomList, $fftype, %ELEMENTS);
	...
	$fftype =  $i->{"ATOMS"}{ pop @atomList }{"FFTYPE"};
...
sub getValenceCoeffs {
    my ($data, $dumpfile) = @_;
    my ($link, $count, $cmd, $error, $result, $mStr, $histData, $tmp1);
    my ($rawData, $binData, $fitFunction, $vars, $rec, $histCat, $tmp, $i);

    $link = new Math::ematica '-linklaunch', '-linkname', 'math -mathlink';

    print "$data->{FILE}:...reading\r";    
    $rawData = readDatFile($data->{FILE});
    printf "$data->{FILE}:%25s\r", " ";

    print "$data->{FILE}:...creating bins\r";
    ($binData, $histData, $histCat) = createBins($rawData);
    printf "$data->{FILE}:%25s\r", " ";

    print "$data->{FILE}:...creating histogram\r";
    createHist($data, $link);
sub createHist {
    my ($data, $link) = @_;
    my ($cmd, $error, $result);
	...
	$cmd = "InputForm[{BestFitParameters, EstimatedVariance} /. " . 
	    "NonlinearRegress[mdata, fitFunc, {x}, {$vars}]]";
	($error, $result) = mathLinkCmd($cmd, 0, $link, $dumpfile);
	if ($error) {
	    showError($error, $link);
	    return 0;
	...
	if ($data->{VTYPE} != 3) {
	    %{ $rec } = %{ $tmp };
	    last;
	} elsif (! exists($rec->{VARIANCE}) || $tmp->{VARIANCE} < $rec->{VARIANCE}) {
	    %{ $rec } = %{ $tmp };
	}
    }
    printf "$data->{FILE}:%25s\r", " ";
    ...
    mathLinkCmd($cmd, 1, $link, $dumpfile);
    $cmd = "datafit = NonlinearFit[mdata, fitFunc, {x}, {$vars}]";
    mathLinkCmd($cmd, 1, $link, $dumpfile);
...
sub createBins {
    my ($data) = $_[0];
    my ($nbins, $range, $bin, $binFile, $i, $max, $histData); 
    ...
    $tot = 0;
    @tmp = sort numerically keys %BINS;
    for $i (@tmp) {
	$tot += $BINS{$i};
    }
