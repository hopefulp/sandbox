...
sub ReadDataFile {
	...
	if ($_ =~ /^\s*(\d+) (atoms|bonds|angles|dihedrals|impropers)\s*$/) {
	    ...
	    $var = uc($1);
	} elsif ($_ =~ /^(Atoms|Velocities|Bonds)\s*$/) {
	    $var = uc($1);
...
sub CreateLAMMPSTrj {
    ...
    my ($i, $atomC, $atom, @dim, $index, $j, $bLen, $cType);
    ...
    $cType = pop @{ $HEADERS->{cType} } if (exists($HEADERS->{cType}));;
    ...
    print $OUTFILE "ITEM: ATOMS id type $cType ix iy iz\n";
...
sub readTail {
	...
	$inputFile = "/home/tpascal/scripts/dat/LAMMPS/in.lammps." . lc($parms->{PARMS}{INPUTTYPE});
    } else {
        $inputFile = "/home/tpascal/scripts/dat/LAMMPS/in.lammps.full";
    }
    $shakeOpts = "";
