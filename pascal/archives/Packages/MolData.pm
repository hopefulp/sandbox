...
sub new {
    ...
    my $data = {
	atomlist  => Packages::LinkedList->new(), # this is the underlying linked list data structure
	atoms     => {}, # this is the accessor (hash) interface to the above linked list
	header    => {},
	mollist   => {}, # underlying data structure
...
my $readBGF = sub {
	...
	$in_data = $_;
		...
		if (defined($molid)) { # one of the atoms already belong to a molecule, so add the rest
		    $newMol = $self->mollist->{$molid};
                    $atom->("molid", $molCounter);
                    for $i (@tmp) {
                        $bond = $self->atoms->{$i};
                        $newMol->addAtom($bond);
                        $bond->("molid", $molCounter);
                    }
		} else { #create a new molecule and add all atoms
		    $molCounter++;
    ...
    $self->header->{"mol2count"} = sprintf("%-5d%-5d%-5d",$atmCounter, $bondCounter, $molCounter);
...
my $writeBondsData = sub {
    ...
    my $OUTDATA = shift;
    my $type = shift;
    my ($count, $currAtom, $bondAtom, $bondCount, %outData, $i, $bondData, $tot);
	...
	while ($count <= $self->count->{"atoms"}) {
		...
		next if (! exists($outData{$i}));
		printf $OUTDATA "%-6s%6d$outData{$i}\n",uc($i), $currAtom->data->index;
	    }
	    $count++;
	    $currAtom = $currAtom->next;
	}
    } elsif ($type eq "mol2") {
	print $OUTDATA '@<TRIPOS>BOND' . "\n";
	while ($count <= $self->count->{"atoms"}) {
	    $bondCount = 1;
	    $bondAtom = $currAtom->data->bondlist->first;
	    while ($bondCount <= $currAtom->data->bondlist->count) {
...
sub write {
    ...
    my $name = shift;
    my $type = lc shift;
    my ($count, $fmt, $currAtom, $outLine, $fields, $i, $cField, $currBond, $val);

    open OUTDATA, "> $name" or die "Cannot write to $name: $!\n";
    print OUTDATA $self->$createHeader($type);
    ...
    # write bond data
    $self->$writeBondsData(\*OUTDATA, $type);
...
# either compresses (cell length < 1) or expands (> 1) cell along specified axis(s)
    ...
    my $axesStr = shift;
    my $factor = shift;
    my ($com, $i, $mol, $newCom, $dim, $offset, $axes, $tmp, $map);
	...
	$tmp = ();
	for $dim (keys %{ $axes }) {
	    $newCom->{$dim} = $com->{$dim} * $factor;
	    $offset->{$dim} = $newCom->{$dim} - $com->{$dim};
	}
	$tmp->{$mol->id} = $mol;
