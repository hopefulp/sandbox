...
my ($HEADERS, $BBOX, $DIM, $calcType, $shellData);
...
sub calcProfile {
    ...
    my ($soluGridAtms, $solvGridAtms, $atmOffset, $totMass);
   ...
   ($GRID, $i, $j) = CreateGrid(\%{ $gridAtms }, 1, \%{ $BOX }, 2.5, 0);
   $soluCells = ();
   ...
   for $i (keys %{ $solvGridAtms }) {
	...
	for $j (keys %{ $SOLVATMS }) {
		...
		if ($calcType == 3) {
		    $DATA = ();
		    undef $DATA;
                    for $k (keys %{ $solvGridAtms->{$j}{MOLECULE} }) {
                        %{ $LAMMPSTRJDATA->{$shellName}{$k} } = %{ $ATOMS->{$k} };
		    }
                }
	    }		
	    $count++;
    ...
    &saveLammpsShellTraj(\%{ $LAMMPSTRJDATA }, \%{ $shellData }, $frameNum, $tot, $BOX, $axis) if ($calcType == 3);
    $LAMMPSTRJDATA = ();
    $ATOMS = ();
    $gridAtms = ();
    $soluGridAtms = ();
...
sub getSoluNeighCells {
	...
	for $y (0 .. $#{ $cells->{Y} }) {
		...
		for $k (@{ $map->{X} }) {
			...
			for $m (@{ $map->{Z} }) {
                            $rec->{ATOMS} = ();
                            #next if (! exists($soluCells->{$k}{$l}{$m}{ATOMS}));
                            #@{ $rec->{ATOMS} } = @{ $soluCells->{$k}{$l}{$m}{ATOMS} };
                            push @{ $currCell->{NEIGHS} }, $soluCells->{$k}{$l}{$m};
...
sub init {
    ...
    print "Initializing...";
    ($solSel,$solvSel,$trjFile,$bgfFile,$rMax,$delR,$trjType,$tSel,$saveFile,$axis,$FF, $solvShells, $offset) =
        ($OPTS{m},$OPTS{v},$OPTS{t},$OPTS{b},$OPTS{r},$OPTS{d},$OPTS{l},$OPTS{s},$OPTS{w},$OPTS{a},$OPTS{f},$OPTS{c},$OPTS{o});
