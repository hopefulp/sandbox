...
job_list=( cp grep grepa grepf mv qsub rename rm rmr dos_fermi msi2pos.pl ls )
...
for sys in `cat $Sys.dat`  	#`ls $frag1 -d1`; `cat $Sys.txt`
	...
	case $job in
	    ...
	    "${job_list[1]}")
		#echo $ofile
		if [ "$string" == "Total energy" ]; then
		    grep "$string" $ofile  | awk '{print $9}'	# Q-Chem
		elif [ "$string" == "Final energy" ]; then
		    grep "$string" $ofile  | awk '{print $4}'
		fi
		#grep F= $w_dir.out
		...
		;;
	    "${job_list[5]}")
		~/bin/changeline_pbs_kdft.pl pbs-kdft.csh $w_dir 1 g1
		;;
	    "${job_list[6]}")
