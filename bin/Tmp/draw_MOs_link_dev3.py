...
FL_MO_types=[]
for fname in list_qchem_ofiles:
    if re.search("P", fname):
        mo_type=0
    elif re.search("C", fname):
        mo_type=1
    else:
        "Error for Molecular type for selective drawing"
        exit(0)
...
for file in list_qchem_ofiles:
        ...
        if MOCoeff_anal==1 and tag_moc=="YES":
            if flag_moc == "NO":
                if re.search(keyword_moc, line) :
                    flag_moc="YES"
                    print "#####################################"
                    print list_qchem_ofiles[fid], ":: MO Levels"
                    continue
                ...
                ###### arrive MOCoefficients block
                    ...
                    if imoc == 1+nbasis:
                        ...
                        for eff_coeff in sorted_moc_block:
                            #print "[atom_kind id angular_mom] mo_id energy MO_Coeff"
                            #print eff_coeff
                            #exit(2)
                            ...
                            if id_mo == id_tmp:
                                ...
                                id_tmp=id_mo
                                #### Save energy for mpl
                                f_list_imo_select.append(id_mo)
        ...
        if NBO_anal==1:
            if nfile==3 and fid == 1:
                ...
                ### found NBO and first line                
                    ...
                    if flag_bd=="NO":
                        if re.search(keyword_nbo_BD, line) and re.search('Ni', line):
                            flag_bd="YES"
    ...
    if print_level >= 2:
        print FL_ene_line
    FL_iMO_select.append(f_list_imo_select)
...
for list_ene, list_imo in zip(FL_ene_select, FL_iMO_select) :
    print list_ene
