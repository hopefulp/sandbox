...
FL_atom_filter_tag=Fl_atom_filter_tag[:]
                ...
                ###### arrived MO Coefficient line in MOC block: read moc line 
                    ...
                    if imoc == 1+nbasis:
                        ...
                        for eff_coeff in sorted_moc_block:
                            ...
                            #print eff_coeff
                            ######## FILTER 3: by FL_nlumo 
                            #### cut valence orbital
                            #print lumo_id_cut
                            if int(eff_coeff[1]) > lumo_id_cut:
                                #### dump MO Coefficient before break
                                #print "last: ", f_list_imo_sel_ab
                                imo_select=f_imo_dump(dump_mo_id_block, FL_MO_atoms[fid], FL_atom_filter_tag[fid])
                                if imo_select:
                                ...
                                break
                            ######## FILTER 4 in display step: by atom-kind
                            #### cut if it's not Ni
                            #### eff_coeff== [['Ni', 'd', '4'], '100', '-0.016', '0.19560']
                            if FL_MO_type[fid] == 'NiCO':   # with CO2
                                #### 
                            ...
                            #### cut if it's not p-orbital for CO2
                            elif FL_MO_type[fid] == 'Ni': # without CO2
                                if eff_coeff[0][0] == 'Ni' or eff_coeff[0][0] == 'P':
                                #if eff_coeff[0][0] == 'Ni' or (eff_coeff[0][0]=='P' and eff_coeff[0][1]=='1'):
                                #if eff_coeff[0][0]=='P' and eff_coeff[0][1]=='1':
                                #if eff_coeff[0][0] == 'N':
                                    pass
                                else:
                                    continue
                                ...
                                #    continue
                            elif FL_MO_type[fid] == 'CO': # for CO2
                                pass
                            else:
...
#### cut lower energy limit for drawing in figure
    ...
    #### change into all levels for Draw_tag=='all'
    if Draw_tag == 'a':
        j=1
        tmp_list=[]
        tmp_elist=[]
        ...
        nlumo_cut=FL_homo_id[fid][0]+Nmax_lumo
        nhomo_cut=FL_homo_id[fid][0]-Nmax_homo
        print nhomo_cut, nlumo_cut
        for ene in list_ene:
            if re.search("\*", ene):
                pass
...
MO_link_hl_ene=[]
#### Make MO-link pair
if nfile >= 2 and 'MOlink_hl_id' in locals() and Link_tag:
