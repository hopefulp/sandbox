...
for($i=0;$i<$pair_size;$i++){
	...
	$tchg_co2=$charges[$j-1];
	$net_co2=$ref_charge{"O"}-$tchg_co2;
 	$ECoulomb+=$coeff*$chg_me*$net_co2/$dist;
...
#  	print $dist,"\n";
	...
	$dist=&Arith::dist($coords[$i][0],$coords[$i][1],$coords[$i][2],$coords[$j-1][0],$coords[$j-1][1],$coords[$j-1][2]);
	$tchg_co2=$charges[$j-1];
	$net_co2=$ref_charge{"C"}-$tchg_co2;
 	$ECoulomb+=$coeff*$chg_me*$net_co2/$dist;
	print "$coeff $chg_me $net_co2 $dist $ECoulomb\n";

	print "Coulomb energy between Me and CO2 $ECoulomb\n";
