...
import sys
from common import *
from clean import d_clean

def d_clean_recursive(dir1,works,prefix, suffix, matches, exclude,excl_fnames, linux_job,new_dir,Lshowmatch,Lall_rm, Lyes):
    ### call d_clean in clean.py
    d_clean(None,works,prefix, suffix, matches, exclude,excl_fnames, linux_job,new_dir,Lshowmatch,Lall_rm, Lyes)
    p_files = os.listdir('.')
    if 'NN18' in p_files:
        print(f'{p_files}')

    for f in p_files:
        if os.path.isdir(f) and not os.path.islink(f): # without link test, it makes an error
            os.chdir(f)
            print(f"####.... enter {f} directory")
            d_clean_recursive(f,works,prefix, suffix, matches, exclude,excl_fnames, linux_job,new_dir,Lshowmatch,Lall_rm, Lyes)
...
def main():
    ...
    if not args.dir1:
        ...
        dir1 = args.dir1
    ### positioned at the working directory
    os.chdir(dir1)
    print(f"####.... enter {dir1} directory")
    d_clean_recursive(dir1,args.works,args.prefix,args.suffix,args.match,args.exclude,args.excluded_files,args.job,args.new_dir,args.match_show,args.all_remove, args.yes)
    return 0
