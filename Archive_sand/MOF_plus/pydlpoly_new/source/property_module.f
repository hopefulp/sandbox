...
c     final averages and fluctuations
     ...
     x    nstep,numacc
        write(nrite,"(1x,120('-'),
     x    /,/,1x,'    step',8x,'eng_tot',7x,'temp_tot',8x,'eng_cfg',
     x    8x,'eng_vdw',8x,'eng_cou',8x,'eng_bnd',8x,'eng_ang',8x,
     x    'eng_dih',8x,'eng_tet',/,1x,'time(ps)',8x,' eng_pv',7x,
     x    'temp_rot',8x,'vir_cfg',8x,'vir_vdw',8x,'vir_cou',8x,
     x    'vir_bnd',8x,'vir_ang',8x,'vir_con',8x,'vir_tet',/,
     x    1x,'cpu  (s)',9x,'volume',7x,'temp_shl',8x,'eng_shl',
     x    8x,'vir_shl',10x,'alpha',11x,'beta',10x,'gamma',8x,'vir_pmf',
     x    10x,'press',/,/,
     x    1x,120('-'))")          

        write(nrite,'(1x,i8,1p,9e15.6,/,1x,0p,f8.3,1p,9e15.6,
     x    /,1x,0p,f8.2,1p,9e15.6)') 
     x    nstep,(sumval(i),i=1,9),
     x    dble(nstep)*tstep,(sumval(i),i=10,18),
     x    timelp,(sumval(i),i=19,27)
        write(nrite,"(/,1x,' r.m.s. ',1p,9e15.6,/,1x,'fluctn. ',
     x    1p,9e15.6,/,9x,9e15.6)") (ssqval(i),i=1,27)
        write(nrite,"(1x,120('-'))")
...
c        write(nrite,"(/,/,1x,'sample of final configuration',/)")
c        write(nrite,"(6x,'i',7x,'x(i)',8x,'y(i)',8x,'z(i)',
c     x    7x,'vx(i)',7x,'vy(i)',7x,'vz(i)',7x,'fx(i)',7x,
c     x    'fy(i)',7x,'fz(i)',/,/)")
c        io=(natms+19)/20
c        
c        do i=1,natms,io
c          
c          write(nrite,"(1x,i6,1p,3e12.4,3e12.4,3e12.4)") 
c     x      i,xxx(i),yyy(i),zzz(i),vxx(i),vyy(i),vzz(i),
c     x      fxx(i),fyy(i),fzz(i)
c          
c        enddo
