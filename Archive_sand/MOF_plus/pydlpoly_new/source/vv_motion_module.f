...
cRS if image tracking is used alternative routines must be applied
        
       if (ltrackimg)then
c        write (*,*) "Tracking images in nvevv_1"
        call images_track(imcon,idnode,mxnode,natms,cell,xxx,yyy,zzz,
     x                    imgx,imgy,imgz)
        if(mxnode.gt.1)
     x    call merge_track(idnode,mxnode,natms,mxbuff,mxibuff,
     x                    xxx,yyy,zzz,buffer,imgx,imgy,imgz,ibuffer)
       else
...
cRS DERBUG DEBUG
c      write (*,*) "DEBUG in nvtvv_b1: Ekin  ", engke
c      write (*,*) "DEBUG            : sigma ", sigma
cRS DEBUG DEBUG
...
cRS   if we use per molecule kinetic energies we need to scale these, too
      if (lmol_ekin) then 
          call molecules_scale_ekin(chit)
      end if
...
cRS if image tracking is used alternative routines must be applied
       ...
       if (ltrackimg)then
c        write (*,*) "Tracking images in nvtvv_b1"
        call images_track(imcon,idnode,mxnode,natms,cell,xxx,yyy,zzz,
     x                    imgx,imgy,imgz)
        if(mxnode.gt.1)
     x    call merge_track(idnode,mxnode,natms,mxbuff,mxibuff,
...
cRS if image tracking is used alternative routines must be applied
       ...
       if (ltrackimg)then
c        write (*,*) "Tracking images in nvtvv_e1"
        call images_track(imcon,idnode,mxnode,natms,cell,xxx,yyy,zzz,
     x                    imgx,imgy,imgz)
        if(mxnode.gt.1)
     x    call merge_track(idnode,mxnode,natms,mxbuff,mxibuff,
...
cRS if image tracking is used alternative routines must be applied
       ...
       if (ltrackimg)then
c        write (*,*) "Tracking images in nvtvv_h1"
        call images_track(imcon,idnode,mxnode,natms,cell,xxx,yyy,zzz,
     x                    imgx,imgy,imgz)
        if(mxnode.gt.1)
     x    call merge_track(idnode,mxnode,natms,mxbuff,mxibuff,
