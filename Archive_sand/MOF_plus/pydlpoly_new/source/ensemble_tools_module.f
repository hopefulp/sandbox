...
c     Exp
c
cRS   added features for per molecule kinetic energy and thermostating
c     
c***********************************************************************
...
c*********************************************************************
      ...
      real(8) engke,sigma,tstep,qmass,chit,conint,scale,taut,scale2
...
cRS DERBUG DEBUG
c      write (*,*) "DEBUG in nvtscale: Ekin  ", engke
cRS DEBUG DEBUG
...
cRS   scale also mol_ekin if system has been scaled
      if (lmol_ekin) then
        scale2 = scale**2
        do i=0,mx_molecules
...
c*********************************************************************
      ...
      integer natms,idnode,mxnode,i,j,iatm0,iatm1
      real(8) getkin,engke
      ...
      if (.not.lmol_ekin) then
      
cRS     ok .. this is the default: do all atoms alltogether
        engke=0.d0
...
cRS     now we do the kinetic energy on a per molecule basis
cRS       the individual energies are stored in the molecule_module array mol_ekin
        ...
        do i=iatm0,iatm1
          j = mol_which(i)
          mol_ekin(j)=mol_ekin(j)+
     x       (0.5d0*weight(i)*(vxx(i)**2+vyy(i)**2+vzz(i)**2))
        enddo

        if (mxnode.gt.1) then
          call gdsum(mol_ekin, mx_molecules, buffer)
        endif
        
        getkin = 0.0d0
        do i=1,mx_molecules
          getkin=getkin+mol_ekin(i)*mol_lamb_ekin(i)
        enddo
