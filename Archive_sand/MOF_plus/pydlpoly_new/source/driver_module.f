...
cRS added ntpatm for recomputing lrcorrection when lambda changes
     ...
     x  ntpter,ntpvdw,ntshl,ntpatm,ntteth,ntcons,numrdf,nsolva,isolva,
...
ccs
        if(time)then
          call timchk(1,tval)
          write(tout,'(a32,f15.3)')'Job time before nlist_driver: ',
     x    tval
        endif
ccs
...
ccs
        if(time)then
          call timchk(1,tval)
          write(tout,'(a32,f15.3)')'Job time after nlist_driver: ',
     x    tval
        endif
ccs
...
ccs
        if(time)then
          call timchk(1,tval)
          write(tout,'(a32,f15.3)')'Job time after force_manager: ',
     x    tval
        endif
ccs
...
c     for npt routines     note: virsrp already includes virlrc
     ...
     x  ntpfbp,ntpmet,ntptbp,ntpter,ntpvdw,ntshl,ntpatm,ntteth,numrdf,
...
c     end of shell relaxation
     ...
     x  ntptbp,ntpter,ntpvdw,ntshl,ntpatm,
     x  ntteth,numrdf,alpha,delr,dlrpot,
