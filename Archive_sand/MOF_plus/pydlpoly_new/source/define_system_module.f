...
ccs   check for keyword gaussian
        elseif(findstring('gaussi',directive,idum))then

         lgauss=.true.
         if(idnode==0)write(nrite,"(/,/,1x,'Electrostatics : ',
     x    'Gaussian charge distribution activated')")
         if(findstring('core', directive, idum))then
            use_core_charge=.true.
            if(idnode==0)write(nrite,"(/,/,1x,'Electrostatics : ',
     x            'Using fixed core charges')")
         end if

        elseif(findstring('all cterms',directive,idum))then
          all_coul=.true.
          if(idnode==0)write(nrite,"(/,/,1x,'Electrostatics :
     x     Exclusion list ignored')")
ccs

crs
        elseif(findstring('sp-basis',directive,idum))then
          lspbasis=.true.
          if(idnode==0)write(nrite,"(/,/,1x,'Electrostatics :
     x     Using SP-Basis')")
...
ccs   print user warning if QEq on but electrostatics off
          if(lgauss)then
            if(idnode==0)write(nrite,
     x      "(/,/,1x,'QEq : Ignoring Gaussian charges')")
ccs
          endif
...
cRS switching          
        elseif(findstring('sw_vdw',directive,idum))then
          
c     factor for the vdw switching
          vdwswitch=dblstr(directive,lenrec,idum)
          lvdwswitch=.true.
          if(idnode.eq.0)write(nrite,
     x    "(/,1x,'switching factor for vdw    ',3x,1p,e12.4)")vdwswitch

        elseif(findstring('sw_chrg',directive,idum))then
          
c     factor for the charge switching
          chrgswitch=dblstr(directive,lenrec,idum)
          lchrgswitch=.true.
          if(idnode.eq.0)write(nrite,
     x    "(/,1x,'switching factor for charge ',3x,1p,e12.4)")chrgswitch

c     switch to vdw force computation via direct derivative of interpolant
        elseif(findstring('derivative vdw interp',directive,idum))then

         lvdwintder=.true.
         if(idnode==0)write(nrite,"(/,/,1x,'VDW : ',
     x    'VDW forces via derivative of 3point interp')")

        elseif(findstring('derivative vdw spline',directive,idum))then

         lvdwspline=.true.
         if(idnode==0)write(nrite,"(/,/,1x,'VDW : ',
     x    'VDW forces via spline interpolation')")
...
cRS   ############### new settings ###############
        ...
        elseif(findstring('dispersion',directive,idum))then

          dispSr6=dblstr(directive,lenrec,idum)
          dispS8 =dblstr(directive,lenrec,idum)
          if(idnode.eq.0)write(nrite,
     x      "(/,1x,'D3 params Sr6 S8',3x,1p,2e12.4)")dispSr6,dispS8
...
crs
      if (lvdwswitch)then
        if ((vdwswitch.gt.1.0d0).or.(vdwswitch.le.0.0d0)) then
      ...
      if (lchrgswitch)then
        if ((chrgswitch.gt.1.0d0).or.(chrgswitch.le.0.0d0)) then
...
ccs/rs
            if(lgauss) then
               qsigma(indatm)=sgmsit(indnam+m)
c               write (*,*) indatm, indnam, m
               if (use_core_charge) core_chge(indatm)=coresit(indnam+m)
            end if             
ccs/rs
...
c***********************************************************************
      ...
      logical lgofr,lzden,lfree,lsolva,lghost, skip_REVOLD
...
cRS if skip_REVOLD zero everything (default in usual pydlpoly MD .. 
cRS           this switch is just to keep the option for orig_run()
      if((keyres.eq.1).and.(idnode.eq.0).and.(.not.skip_REVOLD))then
...
cRS   
c      The original line in DL_poly is this:
c     degfre=dble(3*(ntfree-ntshl)-3-ntcons)+degfre
c      the porblem: degfre already contains the degrees of freedom of the rigid bodies
c      which needs to be added here, however this system subroutine is calles in each 
c      MD_init in pydlpoly and the degrees of freedom start to add up. so instead we added
c      the degrees of freedom of the rigid bodies here explicitly as it is done in the quatbook 
c      routine:  degrot+dble(ngrp)*3.d0
c                degrot: rotational degrees of freedom of the rigid bodies
c                ngrp*3: translational degrees of freedom of the rigid bodies 
      
      degfre=dble(3*(ntfree-ntshl)-3-ntcons)+(degrot+dble(ngrp)*3.d0)
...
c     Initialize fft tables
         ...
         call dfftw_plan_dft_3d(fplan,kmaxd,kmaxe,kmaxf,qqq,qqq,
     x        FFTW_FORWARD,FFTW_ESTIMATE+FFTW_IN_PLACE)
         call dfftw_plan_dft_3d(bplan,kmaxd,kmaxe,kmaxf,qqq,qqq,
     x        FFTW_BACKWARD,FFTW_ESTIMATE+FFTW_IN_PLACE)
c$$$             call fftw3d_f77_create_plan
c$$$     x           (fplan,kmaxd,kmaxe,kmaxf,
c$$$     x            FFTW_FORWARD,FFTW_ESTIMATE+FFTW_IN_PLACE)
c$$$
c$$$             call fftw3d_f77_create_plan
c$$$     x           (bplan,kmaxd,kmaxe,kmaxf,
c$$$     x            FFTW_BACKWARD,FFTW_ESTIMATE+FFTW_IN_PLACE)
