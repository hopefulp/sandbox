...
atomicnumbers = elements.numbers
a2b = 1.0/0.529177
...
class tralyzer2:
    ...
    def __init__(self, pdpf, stage, tstep=None):
        ...
        if ("tstep" in self.traj.attrs.keys()) and (tstep == None):
        ...
        if self.boundarycond > 0:
            if not self.traj.keys().count("cell"):
                ...
                if "nstep" in self.traj["cell"].attrs.keys():
    ...
    def compute_elastic_constants(self, T, first_frame=0, last_frame=None, h0=None):
        ...
        eps_mat = np.array(map(lambda hi : (np.dot(h0m1t, np.dot(hi.transpose(), np.dot(hi, h0m1))) - np.identity(3))/2.0, h))
    ...
    def load_com(self, molname):
        if molname+"_COM" in self.traj.keys():
...
#        return new_map
    ...
    def calc_diffc(self, msd, firstframe, lastframe):
        ...
        tsmsd *= 0.01
        Diffadd = 0
        for i in xrange(firstframe, lastframe-1):
            if  i != 0:
    ...
    def compute_orient(self, molname, atompair, store=True):
        ...
        # NOTE we could use the dtype of the xyz dataype here 
        orient = np.zeros([self.nframes,nmols,3],dtype=self.xyz.dtype)
        if self.fixed_cell:
            celldiag = self.cell.diagonal()
            half_celldiag = celldiag/2.0
        for f in xrange(self.node_id,self.nframes, self.nodes):
