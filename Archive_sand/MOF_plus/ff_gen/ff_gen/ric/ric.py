...
from ._ric import red_int_coords as _ric
...
class RedIntCoords(object):
  ...
  def setup(self, masses):
    ...
    # Not: self.in_bends is [# of stretches, 3] array,
    # but for Fortran subroutines it needs to be transposed.
    nin_bend = 0
    ...
    nout_bend = 0
    self._ric.ric_def_out_bends = np.transpose(self._out_bends)
    ...
    nlin_bend = 0
    self._ric.ric_def_lin_bends = np.transpose(self._lin_bends)
    ...
    ntorsion = 0
    self._ric.ric_def_torsions  = np.transpose(self._torsions)
    ...
    self._ric.bmat     = np.zeros((3*natom,nric))
    self._ric.bmat_inv = np.zeros((3*natom,nric))
    ...
    self._ric.ric_hessian       = np.zeros((nric,nric))
