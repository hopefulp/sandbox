...
/* ----------------------------------------------------------------------
   Contributing author: Rochus Schmid (Ruhr-Universitaet Bochum) and
                        Hendrik Heenen (Technical University of Munich)
...
#include <stdlib.h>
#include <iostream>
#include "improper_umbrella_harmonic.h"
#include "atom.h"
#include "comm.h"
#include "neighbor.h"
#include "domain.h"
...
ImproperUmbrellaHarmonic::ImproperUmbrellaHarmonic(LAMMPS *lmp) : Improper(lmp)
{
  writedata = 1;
}
...
ImproperUmbrellaHarmonic::~ImproperUmbrellaHarmonic()
{
  if (allocated) {
...
void ImproperUmbrellaHarmonic::compute(int eflag, int vflag)
{
  int i1,i2,i3,i4,n,type;
  double eimproper,f1[3],f2[3],f3[3],f4[3];
  double vb1x,vb1y,vb1z,vb2x,vb2y,vb2z,vb3x,vb3y,vb3z;
  double dw,w,domega,c,a,s,projhfg,dhax,dhay,dhaz,dahx,dahy,dahz,cotphi;
  ...
  for (n = 0; n < nimproperlist; n++) {
    ...
    // RS Remark: w' is angle between ar and hr, but hr is perpendicular to IJK plane
    //            thus, we are interested in w = 90deg-w'
    //            c = cos(w') = sin(w) and s = sin(w') = cos(w)
    ...
    dw = w-w0[type];
    if (eflag) eimproper = 0.5 * kw[type] * dw*dw;
    a = kw[type]*dw;
    
    /*
    if (kw[type] > 0.0) {
        std::cout << "\nDEBUG DEBUG\n";
        std::cout << i1+1 << " " << i2+1 << " " << i3+1 << " " << i4+1 << "\n";
        std::cout << w/MY_PI*180.0 << " " << eimproper << "\n";
    }
    */
...
void ImproperUmbrellaHarmonic::allocate()
{
  allocated = 1;
...
void ImproperUmbrellaHarmonic::coeff(int narg, char **arg)
{
...
void ImproperUmbrellaHarmonic::write_restart(FILE *fp)
{
...
void ImproperUmbrellaHarmonic::read_restart(FILE *fp)
{
  allocate();
...
void ImproperUmbrellaHarmonic::write_data(FILE *fp)
{
