...
def calc_train_images(images, des_obj, HL, Elist, flist, ncore, amp_pot=None):
    ...
    return
### AMP job 2: Test
def calc_test_images(test_images, amp_pes, Lhack_f, title, suptitle,ncore, Lgraph,Lga=False, val_id=None,na_in_mol=1,Ltwinx=None):
    ...
    if Lgraph:
        ...
        hl = calc.model.parameters.hiddenlayers[test_images[0][0].symbol]  # retuns hl as tuple
        ...
        if Lga:
            ...
            with open("ga_fit.txt", 'w') as f:
                f.write(' '.join(str(nodes) for nodes in hl))
                f.write(f" {score}\n")
            os.system("cat ga_fit.txt >> ../ga_gen_fit.txt")  # GA checks this file
...
def amp_jobs(fdata,job,descriptor,Lhack_f,amp_inp,HL,E_conv,f_conv_list,ncore,na_mol,ndata,ntrain,dstype,dslist,Lgraph,Ltwinx):
#def amp_jobs(fdata,job,descriptor,amp_inp,HL,E_conv,f_conv_list,ncore,na_mol,ndata,ntrain,dstype,dslist,Lgraph,Ltwinx):
    ...
    ### JOB == TRAINING
        ...
        calc_train_images(tr_images, descriptor, HL, E_conv, f_conv_list, ncore, amp_pot=amp_inp)
    ...
    ### JOB == TEST
        ...
        print(f"data test/train {ntest}/{ntrain} in {whereami()}/job==te")
...
def main():
    ...
    ### if not MD, it's training/test
        ...
        amp_jobs(fname,args.job,des_obj,args.hack_force,args.pot,args.hidden_layer,args.e_conv,args.f_conv,args.ncore,args.mol_atoms,args.ndata_total,args.ndata_train,args.data_s_type,args.data_s_list,args.g,args.twinx)
