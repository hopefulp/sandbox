...
def run_amp_wrapper(fin, job_submit, Lcheck_string, Ltest_only, hl, qname):
    '''
    ...
    p1 = Parameters({'jsubmit':job_submit, 'ampjob':'tr',  'ncore':'10', 'max_iter':2000, 'hl':str_hl, 'elimit':'0.0001',
    ...
    if os.path.isfile("amp.amp") or os.path.isfile("amp-untrained-parameters.amp"):
        ...
        if job_submit == 'qsub':
            ...
            print(ampstr())
            if not Ltest_only and not Lcheck_string:
                os.system(ampstr())
    ...
    while True:
        time.sleep(30)
        print(f"test is going until finding '{amp_ini.ampout_te_f_chk}'")
        if os.path.isfile(amp_ini.ampout_te_f_chk):
            break
    print("Job is Done")
