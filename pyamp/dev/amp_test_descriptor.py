...
def chk_descriptor(fdata,job,Ltest_f,amp_inp,Lload_amp,HL,E_conv,f_conv_list,ncore,na_mol,ndata,ntrain,dstype,dslist,Lgraph,Gs,Ltwinx):
    ### parameter file
...
def main():
    ...
    parser.add_argument('-j', '--job', default='tr', choices=['tr','pt','chk'], help='job option:"train","plot","check"')
    ...
    ### Gaussian Descriptor
    descriptor = parser.add_argument_group(title='Gaussian')
    descriptor.add_argument('-gs','--gaussian_symmetry', help='if None, gs is calculated in script')
    ...
    chk_descriptor(fname,args.job,args.test_force,args.pot,args.load_pot,args.hidden_layer,args.e_conv,args.f_conv,args.ncore,args.mol_atoms,args.ndata_total,args.ndata_train,args.data_s_type,args.data_s_list,args.g,args.gaussian_symmetry, args.twinx)
    return
