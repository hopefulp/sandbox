...
build   = MyClass('build')
fconv   = MyClass('fconv')
dftalpha= MyClass('dftalpha')


build.build_struct  = "build STRUCT.fdf\
                    \n\t\tNow two particle dissociation is equipped\
                    \n\t\timported module: pycommon/libsci generate_distances\
                    \n\t\t    sampling many around peak\
                    \n\t\tOptions:\
                    \n\t\t    -a, --atoms           two atom list\
                    \n\t\t    -rmme, --rminmaxeq    minimum, maximum, equilibrium distances\
                    \n\t\t    -n, --npoints         # of distances, 10 (default) between r_eq and r_max\
                    \n\t\t    -n, --npoints_left    # of distances, 3 (default) between r_min and r_eq\
                    \n\t\t    -o, --outdir          output directory (geometries)\
                    \n\t\t    -p, --prefix          filename prefix (mol) -> mol_001.fdf\
                    \n\t\tUsage::\
                    \n\t\t    build_struct.py -a H H -rmme 0.5 10 0.74 -np 15 -p H2"

dftalpha.siesta_half="siesta executable (binary) generated by RG Lee"
dftalpha.siestagap  ="HOMO(VBM)-LUMO(CBM) calculator generated by RG Lee"
...
fconv.poscar2fdf    ="convert POSCAR format to fdf\
                    ...
                    "
classobj_dict={'FILE_CONV': fconv, 'BUILD': build, 'DFTALPHA': dftalpha} 
...
def classify(Lclassify, work, class_name, job, fname,HL, elimit, nc, Lgraph):
    ...
    #print(f"Globals {globals()}") # 'build':{}, 'fconv':{}, 'dftalpha':{}
    #print(f"MyClass {MyClass} instance {MyClass.instances}")
    if sort_dir:
        print("Directories:: ")
        if not Lclassify:
            for f in sort_dir:
            ...
            for instance in MyClass.instances:
                for gkey in globals().keys():
                    #print(f"instance {instance.name}, gkey {gkey}")
                    if gkey == instance.name:
                        break
                    ### go over to next gkey
                    else:
                        pass
                ...
                ### work --> detailed
                    ...
                    for ckey in ckeys:
                        print(f"    {ckey}\t:: {globals()[gkey].__dict__[ckey]}")
