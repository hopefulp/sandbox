...
def amp_train(ntotal, images, amp_pes, HL, E_conv, f_conv, Lgraph, ncore, n_mol, Ltwinx):
    #total_images = ase.io.read(fdata, index=':')    # can read extxyz, OUTCAR, 
    #images_sets = Images(total_images, nsets=data_int)
    print("data training:total sets %d/%d" % (len(images), ntotal))
    ...
    f_write('CONTCAR', HL, E_conv, f_conv, rmserr, max_res, job)
    return
...
def data_gathering(dt, dm, fname, job):
    total_images    = ase.io.read(fname, index=':')    # extxyz, OUTCAR, ':' should be
...
def main():
    ...
    ### data selection
    parser.add_argument('-dt','--data_type', default='set', choices=['set','sort','interval','pick'], help='data interval list')
    parser.add_argument('-dm','--data_mine', type=int, nargs='*', help='data selection')
    ...
    ntotal, train_im, test_im = data_gathering(fname,args.data_type, args.data_mine, args.job)

    if args.job == 'tr':
        amp_train(ntotal,train_im,args.pot,args.hidden_layer,args.e_conv,args.f_conv,args.g,args.ncore,args.nmol,args.twinx)
    elif args.job == 'te':
        amp_test(ntotal,len(train_im),test_im,args.pot,args.hidden_layer,args.e_conv,args.f_conv,args.g,args.ncore,args.nmol,args.twinx)
    return
