...
version = '20160712'
logging.basicConfig(level=logging.WARNING, format="%(asctime)s %(name)s: %(message)s", datefmt="%y-%m-%d %H:%M")
logger = logging.getLogger("Trj")


def get_line(trj_file):
    assert isinstance(trj_file, str)
...
class Trj:
    ...
    def __init__(self, trj_file, silent=True):
        ...
        dirname = os.path.dirname(self.trj_file)
        filename = os.path.basename(self.trj_file)
        self.data_file = dirname + "." + filename + ".summary.pickle"

        if os.path.exists(self.data_file):
            logging.info("Loading information from %s" % self.data_file)
            with open(self.data_file, 'rb') as f:
                timemark = pickle.load(f)
                if timemark == time.ctime(os.path.getmtime(self.trj_file)):
                    logger.info("Trajectory file %s is not modified after scan. Proof-reading.." % self.trj_file)
                    ff = pickle.load(f)
                    ...
                    logger.warning("Trajectory file %s is modified after scan. Rescanning.." % self.trj_file)

        def scan():
            header = []
            for line in get_line(self.trj_file):
    ...
    def load(self, force=False):
        ...
        if not self.nheader or not self.natoms:
            logger.warning("Trajectory file %s seems to be empty." % self.trj_file)
            return

        dumpatom = get_line(self.trj_file)
        ...
        while 1:
            ...
            # if not self.silent:
            sys.stdout.write('\rGetting information from LAMMPS trj file %s .. Timestep: %d' % (self.trj_file, t))
            sys.stdout.flush()
    ...
    def dump(self, requested_ts=(), desc=""):
        ...
        :return: None.
        """
        assert isinstance(requested_ts, list)

        if not desc:
            desc = "Dumping coordinates"

        if not self._is_loaded:
            logger.warning("LAMMPS trajectory file not loaded. Use load() function first.")
            return
        ...
        for t in tqdm.tqdm(self.timesteps, ncols=120, desc=desc):
            tinfo = {}
            chunk = [next(dumpatom) for i in range(self._nchunk)]
            if requested_ts:    # if requested_ts specified, those timesteps only will be stored.
            ...
            for line in coords:
                atominfo = {}
                coord = line.split()
                atomid = int(coord[self._dump_keywords_r['id']])
                atomtype = int(coord[self._dump_keywords_r['type']])
                atominfo['id'] = atomid
                atominfo['type'] = atomtype
                for index, i in enumerate(coord[2:]):
                    atominfo[self._dump_keywords[index + 2]] = float(i)

                tinfo[atomid] = atominfo
            self.coord[t] = tinfo
