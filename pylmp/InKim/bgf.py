...
def pbc_distance(atom1, atom2, dim):
    import numpy as np

    x = np.array([atom1.x, atom1.y, atom1.z]); y = np.array([atom2.x, atom2.y, atom2.z]); dim = np.array(dim)
    delta = np.abs(x - y)
...
class BgfFile:
    ...
    addAtom(self,atom,index=None):
        ...
        if atom.aNo in self.a2i:   # aNo exists, need to renumber this atom:
    ...
    renumber(self,start=1):
        ...
        for i in range(self.natoms):
            ...
            for j in range(len( myConectList )):
                ...
                if connectedAtom in old2new_aNo:
