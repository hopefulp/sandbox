...
bin_dist = np.arange(2.5, 3.6, dr)
bin_angle = np.arange(0 + db, 100, db)
dist = []
angle = []

if not (os.path.exists("hbond.dist.pickle") and os.path.exists("hbond.angle.pickle")):
    # pickle load
    ...
    # store
    with open("hbond.dist.pickle", 'wb') as f:
        print("writing pickle hbond.dist.pickle")
        pickle.dump(dist, f)
    with open("hbond.angle.pickle", 'wb') as f:
        print("writing pickle hbond.angle.pickle")
        pickle.dump(angle, f)
else:
    with open("hbond.dist.pickle", 'rb') as f:
        print("loading pickle hbond.dist.pickle")
        dist = pickle.load(f)
    with open("hbond.angle.pickle", 'rb') as f:
        print("loading pickle hbond.angle.pickle")
        angle = pickle.load(f)
