...
if sys.version_info < (3, 0):
    print("ERROR: Must be using Python 3.0 or higher.")
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    for t in tqdm.tqdm(chunks, ncols=120):
        x = run_hb(mybgf, chunks[t])

    '''
    # multi processors usage
    actual_workers = workers - 2
    print("Start multiprocessing.. Will use %d processors for calculation." % actual_workers)
    with concurrent.futures.ProcessPoolExecutor(max_workers=actual_workers) as exe:
        fs = {exe.submit(run_hb, mybgf, k, chunks[k]) for k in chunks.keys()}
        #kwargs = {'total': len(fs), 'unit': 'run_hb', 'unit_scale': True, 'leave': True, 'ncols': 120, 'desc': "Calculating Hbonds"}
        kwargs = {'total': len(fs), 'unit': 'run_hb', 'leave': True, 'ncols': 120, 'desc': "Calculating Hbonds"}

        # from https://renzo.lucioni.xyz/parallel-progress/
