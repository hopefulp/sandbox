...
for { set fr 1 } { $fr <= $n_frames } { incr fr } {
    ...
    ### REMARK ### In short nanotube cases with height < radius, principal axis is the first component of the MI matrix
	set A [orient $cnt [lindex $I 0] {0 0 1}]
	...
	set str "[format %d $fr][format %8d [$inside num]][format %8.3f $avg_dist][format %8.1f $h]"
	puts $outfile $str
	flush $outfile
}
puts " Done."
