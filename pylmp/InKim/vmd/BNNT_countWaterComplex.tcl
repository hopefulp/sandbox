...
for { set fr 1 } { $fr <= $n_frames } { incr fr } {
	...
	# select atoms in cnt
	set inside [atomselect top "(x-$cx)**2+(y-$cy)**2 < $avg_dist**2 and z > $zmin-$margin and z < $zmax+$margin and type OW"]
	#puts "$fr $cx $cy $h [expr sqrt($avg_dist_sq)] [$inside num]"
	set zm [expr "$zmin-$margin"]
	set zM [expr "$zmax+$margin"]
	set str "[format %d $fr][format %8d [$inside num]][format %8.3f $avg_dist][format %8.1f $h]\t(x-[format %8.3f $cx])**2+(y-[format %8.3f $cy])**2 < [format %8.3f $avg_dist]**2 and z > [format %8.3f $zm] and z < [format %8.3f $zM] and type OW"
	puts $outfile $str
	flush $outfile
}
puts " Done."
