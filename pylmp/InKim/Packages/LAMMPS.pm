...
my $scripts_dir = "/home/noische/scripts"; #change here as necessary
@ISA = qw(Exporter);
@EXPORT = ();
...
sub determineIfScaled {
    ...
    print "coordinates...";
    $OPTS->{hasvel} = 1 if (exists($DATA->{ATOMS}{1}{XVEL}));
}
...
sub ReadDataFile {
	...
	if ($_ =~ /^\s*(\d+) (atoms|bonds|angles|dihedrals|impropers)\s*$/) {
	    ...
	    $var = uc($1);
	} elsif ($_ =~ /^(Atoms|Bonds|Angles|Torsions|Impropers|Velocities)\s*$/) {
	    $var = uc($1);
...
sub getFormat {
    ...
    open LMPFILE, $lmpFile or die "ERROR: Cannot open $lmpFile: $!\n";
    while (<LMPFILE>) {
	chomp;
	if ($_ =~ /ITEM: ATOMS\s+(.*)$/) {
	   $fList = $1;
    ...
    close LMPFILE;
    die "ERROR: $lmpFile if not valid!\n" if (! $fList);
    $fields = getLineFormat($fList);
    die "ERROR: No valid fields found while searching \"@{ $fList }\"\n" if (! @{ $fields });
...
sub readTail {
	...
	while (<TAIL>) {
	    ...
	    $inStr =~ s/all npt.*$/all npt 300.0 300.0 100.0 x 1.0 1.0 2000.0 y 1.0 1.0 2000.0 couple xy/ 
...
#		    "$parms->{PARMS}{cut_coul} $parms->{PARMS}{cut_vdw}";
    ...
    if ($parms->{PARMS}{isTriclinic} and $parms->{PARMS}{OPTIONS}{PERIODICITY}) {
