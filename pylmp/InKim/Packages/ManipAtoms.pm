...
@EXPORT_OK = qw(ImageAtoms ScaleAtoms UnwrapAtoms FindElement CenterSystem GetAtmList 
		...
		BuildAtomSelectionString SelectAtoms);
...
sub GroupAtomsByField {
    ...
    my ($i, $j, $FIELD, $index, @sorted, $resid, $resNum, $MOLS, $tmp, $hasSelect);
    ...
    for $i (0 .. $#sorted) {
	for $j (sort {$a<=>$b} keys %{ $FIELD->{$sorted[$i]} }) {
	    $index++;
	    $atoms->{$j}{INDEX} = $index;
