...
def crosslink(bgf_file, ff_file, suffix, init_script_original_file, crosslink_script_original_file, t, ratio, out_file):
	...
	f_out_file.write("\tNumber of primary, secondary, and tertiary amine groups are investigated and updated.\n")

	# check ratio
	n_initAvailAmineSites = AmineGroupInfo[0] + AmineGroupInfo[1]
	f_out_file.write("\tThere are " + str(n_initAvailAmineSites) + " available sites for crosslinking." + "\n")
	...
	# first run
	t1 = time.time()
	runLammpsParallel = lammps_parallel + " -in " + initial_in_file + " -log " + os.path.join(temp_dir, suffix + ".log")
	...
	for i in range(1, t + 1):
		...
		runLammpsParallel = lammps_parallel + " -in " + input_file + " -log " + sname + ".log"
		os.system(runLammpsParallel)
		...
		if len(NN_dist_ok) > 0:
			...
			l_amines = bgftools.getAmineGroupInfo(clusterBGF)
			n_allAmineSites = l_amines[0] + l_amines[1]
			r = float(n_crosslink) / float(n_initAvailAmineSites)
			if r > ratio:
				f_out_file.write("**** Requested degree of cross-link reached ****")
				NN_dist_ok = [];
				endFlag = True
			...
			for pair in NN_dist_ok:
				...
				xlinkerBGF = bgftools.rotateBGF(xlinkerBGF, 1, 3, vec, 0)	# 0 means return as BgfFile
			...
			runLammpsParallel = lammps_parallel + " -in in.minimization -log minimization.log"
