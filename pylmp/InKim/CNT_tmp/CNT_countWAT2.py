...
def getVelocity(bgf_file, trj_file, n_step, silent=False):
	...
	while 1:
		...
		### apply periodic condition	REMARK: wrap first and then rotate CNT
		...
		### transpose for "all atoms in BGF": move COM of CNT to the center of the box from the origin
		# com of CNT
		Mx = 0; My = 0; Mz = 0;
		for atom in myBGF.a:
			if "CNT" in atom.rName:
		...
		### apply periodic condition	REMARK: rotate CNT first and put water
		#myBGF = bgftools.periodicMoleculeSort(myBGF, 0, True)
	...
	ftemp = open("countWater2.profile", 'w')
	for i in l_data:
		output = str(i[0]) + '\t' + str(i[1]) + '\t' + str(i[2]) + '\t' + str(i[3]) + '\n'
		ftemp.write(output)
	ftemp.close()
	print("numbers of water molecules are written in countWater2.profile ..Done.")
