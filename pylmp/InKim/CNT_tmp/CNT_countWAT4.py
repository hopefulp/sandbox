...
def getVelocity(bgf_file, trj_file, n_step, silent=False):
	...
	ftemp = open("countWater4.profile", 'w')
	ftemp.write(str(sys.argv) + "\n")

	curr_dir = os.path.abspath(".")
	temp_dir = curr_dir + "/countWAT4/"
	print(temp_dir)
	...
	while 1:
		...
		for atom in myBGF.a:
			atom.x = atom.x - Mx + boxsize[0]/2
			atom.y = atom.y - My + boxsize[1]/2
			atom.z = atom.z - Mz + boxsize[2]/2
		...
		for atom in myBGF.a:
			...
			atom.y = float(Uv[1])	
			atom.z = float(Uv[0])	
		boxsize_prime = np.transpose(np.dot(Ut,np.transpose(np.array(boxsize))))
		...
		# move atoms to box center
		for atom in myBGF.a:
			atom.x = atom.x - Mx + boxsize_prime[0]/2
			atom.y = atom.y - My + boxsize_prime[1]/2
			atom.z = atom.z - Mz + boxsize_prime[2]/2
...
# for CNT atoms, calculate some properties
		...
		#for atom in myBGF.a:
		#	atom.x += myBGF.CRYSTX[0]/2
		#	atom.y += myBGF.CRYSTX[1]/2
		#	atom.z += myBGF.CRYSTX[2]/2

		myBGF = bgftools.periodicMoleculeSort(myBGF, 0, boxsize_prime)	##### testestestest
		...
		output = str(timestep) + '\t' + str(len(aNo_WAT_O_in_CNT)) + '\t' + str(radius_CNT) + '\t' + str(min_z_CNT) + '\t' + str(max_z_CNT) + '\t' + str(z_diff) + '\t' + str(CNT_orientation) + '\t' + str(height_CNT) + '\t' + str(len(aNo_CNT)) + '\n'	# debug
		ftemp.write(output)
