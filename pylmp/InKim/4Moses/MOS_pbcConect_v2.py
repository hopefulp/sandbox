...
Connects atoms near pbc in silica-amine system without surface OH. (version 2)
Connects Si-O and Si-H over pbc boundary.
"""
...
for atom in mybgf.a:
	...
	for atom2 in mybgf.a:
		if not "Si" in atom2.ffType:
			continue;
		if len(atom2.CONECT) == 4:
			continue;
...
print("    {0:<4d} atoms found for Si-H connection.".format(len(pairs2)))
...
for atom in mybgf.a:
	if "Si" in atom.ffType:
		if len(atom.CONECT) != 4:
			#nu.warn("Defect on Si connection found: " + str(atom.CONECTline()) + " . Check BGF file.")
