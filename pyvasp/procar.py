...
def analyze_procar(atom_list, poscar, kpoints, procar):
    ...
    nheadline = 3
    print_test = ''
    ### atom index from 1 following PROCAR
    with open('PROCAR', 'r') as f:
        lines = f.read().splitlines()      # load all the lines in PROCAR: will charge memory
        state = lines[1].split()
        ...
        tag_atom = 0
        ### Does not need to load all the lines
        for line in lines:
            if i <= 2:
                i += 1
                continue
            if not line.strip():
                ...
                lele = line.strip().split()
            
            ### atom block
            if tag_atom:
                if lele[0].isdigit():
                    if int(lele[0]) in atom_list and 0.5 < float(lele[10]):
                        print(f"{lele[0]} {lele[10]} in ({k}, {b})")
                ### end of atom line then start k,b head line
                else:
                    tag_atom = 0
            ### each head lines for k, b
            else:
                if lele[0] == 'k-point':
                    k, w = lele[1], lele[8]
                elif lele[0] == 'band':
                    b = lele[1]
                elif lele[0] == 'ion':  # start atom in the next line
                    tag_atom = 1
...
def main():
    ...
    analyze_procar(atom_index, args.poscar, args.kpoints, args.procar)
