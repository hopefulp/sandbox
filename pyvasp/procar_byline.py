...
def analyze_procar(atom_list, poscar, kpoints, procar):
    ...
    with open('PROCAR', 'r') as f:
        lines = f.read().splitlines()      # load all the lines in PROCAR
        state = lines[1].split()
        ...
        for k in range(nkpoints):
            ...
            for b in range(nbands):
                ...
                # to check PROCAR format
                print(f"{lines[i]} labels in k, band ({k}, {b})")          
                for ia in range(1, nions+1):
                    paband = lines[i+ia].strip().split()
                    #print(f"{paband[0]} {paband[10]}")
                    if int(paband[0]) in atom_list and 0.5 < float(paband[10]) :
                        print(f"{paband[0]} {paband[10]} in {k} {b}")
                ### skip total line at each k, b and empty line
                i += nions + 0
                if re.search('b', print_test): break
            ### for k-block spacing
            i += 0 
            if re.search('k', print_test): break
    return 0 
