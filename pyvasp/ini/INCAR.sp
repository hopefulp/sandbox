...
# Running Parameters
########################
 #ISIF   = 2          ! (0)= MD, (2)= Const_cell, (3)= Relax_cell(ENCUT= ENMAX * 1.3) comment out (SP)
                     ! (ISIF >= 3 : Hessian, Elastic constants w/ IBRION=5,6)
 IBRION = -1          ! (-1=NoUpdate, SP, DOS), (0=MD), (1=NEB,RMM-DIIS), (2=CG/Default), (3=Quick-Min)
                     ! (5,6)=Frequency, Hessian, Elastic, (7,8)=Piezoelectric constants
 NSW    = 0          ! optimization step. (static SCF for DOS (SP): NSW= 0 (0), 1 for ZPE)  
 #POTIM  = 0.3        ! (fs) *Frequency calculation(POTIM < 0.02(0.015)), (MD = 1.0 fs), Default=0.5
...
###########################
 ...
 NWRITE = 3          ! write if something wrong. 4: for debugging.
 LPETIM = F          ! write-flag & timer
 LWAVE  = .F.        ! write WAVECAR.
 LCHARG = .T.        ! write CHGCAR & CHG .T. for DOS, Band
