...
if ( ! -d "$PBS_O_WORKDIR/$oldjob") then
    echo "There is not $oldjob directory" >> $log_file
    exit
...
cp $PBS_O_WORKDIR/$oldjob/POTCAR   	$PBS_O_WORKDIR/$wdir
cp $PBS_O_WORKDIR/$oldjob/CONTCAR  	$PBS_O_WORKDIR/$wdir/POSCAR
cp $PBS_O_WORKDIR/incar.dos$mol	 	$PBS_O_WORKDIR/$wdir/INCAR
cp $PBS_O_WORKDIR/$oldjob/CHGCAR   	$PBS_O_WORKDIR/$wdir
cp $PBS_O_WORKDIR/$oldjob/WAVECAR  	$PBS_O_WORKDIR/$wdir
