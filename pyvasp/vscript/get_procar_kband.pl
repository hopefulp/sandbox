...
@density=qw(0.004 0.000 0.2 0.2);
#@density=qw(0.003 0.003);

#@atoms=qw(1 3);	# for CO2
@atoms=qw(1 2 25 26); 	# for MOF and CO2
...
#    	print  $energy,"\t", $kpoint,"\t",$band,"\t",join("  ",@pdos),"\t", join("  ",@ldos),"\n";
	$diff=abs($pdos[2]-$pdos[3]);
	if($pdos[2] >= $density[2] and $pdos[3] >= $density[3] and $diff >= 0.005){
    	    print  $energy,"\t", $kpoint,"\t",$band,"\t", $pdos[0],"\t", $pdos[1],"\t", $pdos[2],"\t", $pdos[3], "\n";
	}
    }
