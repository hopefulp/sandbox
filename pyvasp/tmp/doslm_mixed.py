...
from libdoscar import read_doscar
...
def extract_doscar(doscar, ofile, alist02d, eshift, l, m, Lplot, plot_dict):
    ...
    for alist0 in alist02d:
        dos_data = read_doscar("DOSCAR", atom_indices=alist0, l=l)
        ene = dos_data[:, 0]
        dos2d.append(dos_data[:, 1])
...
def extract_doscar_orig(doscar, ofile, alist02d, eshift, l, m, Lplot, plot_dict):
    '''
    ...
    dos     = []
    itatom   = 0     # index for total atom count for x_ene=[]

    ### DOSCAR number of column
    if Lspin:
        nlm = 18
    else:
        nlm = 9     # 0:ene, 1:s, 2-4:p, 5-9:d
    ...
    ### AI version
     # Skip header
    n_header = 6
    n_atoms = int(lines[0].split()[0])  # or parse from POSCAR/OUTCAR
    n_ene = int(lines[n_header].split()[2])
    
    # Read energy grid from total DOS block
    energy = np.loadtxt(lines[n_header+1 : n_header+1+n_ene], usecols=0)

    
    # Start index for PDOS blocks
    pdos_start = n_header + 1 + n_ene + 1
    ###/AI
    ...
    for alist0 in alist02d:
        ...
        if not ofile:
            ...
            fname = ofile[idos]
        ### obtain pdos2d[ene][sum of atoms, l, m]
        #pdos2d  = []
    
        # Prepare array for summed DOS: for one groups
        summed_dos = np.zeros(n_ene)

        for i in arr_atoms: # i is atom index in total system
            #block_start = nheadline +  i *(1 + nene)
            block_start = pdos_start + i * (n_ene + 1)
            #print(f"block_start {block_start} nene {nene} i {i} in total lines {len(lines)}")
        ...
        '''
            ...
            for i in range(block_start, block_start+1+nene):
                ...
                if i == 0:
                    dos.append(float(eles[1]))  # for TDOS?
                ### for pdos:2D # update spin case
                else:### initialize pdos2d at first
                    if f_pre == 'pdos':

                    if not pdos2d:
                        pdos2d = [ [ 0.0 for x in range(len(eles)-1) ] for y in range(int(nene)) ]       
