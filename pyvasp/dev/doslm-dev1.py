...
def extract_doscar_1list(doscar, ofile, alist0, idos, Ldoserr, eshift, l, m, bheadline, ngrid):
    '''
    works for one list
    alist0      start from 0
...
### loop depending on number of pdos file
    ...
    if 't' in job:
        ...
        x, y, legend = extract_doscar_1list(doscar,ofile,[-1],idos,Ldoserr, Eshift,l, m, bheadline, ngrid)
        idos += 1
        ys.append(y)
        legends.append(legend)
    for i in range(len(alist02d)):
        x, y, legend = extract_doscar_1list(doscar,ofile,alist02d[i],idos,Ldoserr,Eshift,l,m,bheadline, ngrid)
        ys.append(y)
        legends.append(legend)
    ### plot here
    if Lplot:
