...
### to use anaconda not-base module, comment out shebang

import argparse

import amp                          # to check module file
from amp import Amp
...
### dict={'module name': 'nickname'}
envs={'ampg2offset': 'offset'}

### Amp job 1: Train Images 
def calc_train_images(images, mod, HL, Elist, f_conv, f_coeff, ncore, Lload_amp, des_obj, amp_pot=None):
    ...
    ### Amp Module check
    mod_file = amp.__file__
    print(f"module file = {mod_file}")
    env = "base"
    if "envs" in mod_file:
        lmod_file = mod_file.split('/')
        env = envs[lmod_file[lmod_file.index("envs")+1]]
        print(f"env = {env}")
    ### Amp Module selection
    if env == 'offset':
        print(f"module = {env} is running")
        from amp.descriptor.gaussian import Gaussian
        calc = Amp(descriptor=Gaussian(), model=NeuralNetwork(hiddenlayers=Hidden_Layer), cores=cores)
    else:
        print(f"module = {mod} is running")
        if mod == 'orig':
            from amp.descriptor.gaussian import Gaussian
            calc = Amp(descriptor=Gaussian(), model=NeuralNetwork(hiddenlayers=Hidden_Layer), cores=cores)
...
def amp_jobs(fdata,mod,job,Ltest_f,amp_inp,Lload_amp,HL,E_conv,f_conv_list,ncore,na_mol,ndata,ntrain,dstype,dslist,Lgraph,Ltwinx,descriptor=None):
    ...
    ### JOB == TRAINING
        ...
        calc_train_images(tr_images, mod, HL, E_conv, f_conv, f_coeff, ncore, Lload_amp,  amp_pot=amp_inp, des_obj=descriptor)
...
def main():
    ...
    parser.add_argument('-m', '--mod', default='offset', choices=['orig', 'mydes'], help='select which module will be used')
    ### Descriptor group
    mygs_group = parser.add_argument_group(title="Descriptor generator")
    mygs_group.add_argument('-des', '--descriptor', choices=['gs','zn','bs'], help="test new descriptor")
    mygs_group.add_argument('-pf', '--param_function', default='log10', choices=['log10','powNN'], help="function for parameter interval")
    mygs_group.add_argument('-pmm', '--param_minmax', nargs=2, default=[0.05, 5.0], type=float, help="min, max for param interval")
    mygs_group.add_argument('-pn', '--nparam', type=int, default=5, help="num of parameters for descriptor")
    mygs_group.add_argument('-c', '--cutoff', type=float, default=6.5, help="initialize cutoff distance here")
    ### Neural Network
    ...
    parser.add_argument('-hl', '--hidden_layer', nargs='*', type=int, default=[4], help='Hidden Layer of lists of integer') # 10, 10 | 8, 8, 8
    parser.add_argument('-el', '--e_conv', nargs='+', default=[0.001,0.003], type=float, help='energy convergence limit per atom\not image')
    parser.add_argument('-fl', '--f_conv', nargs='+', default=[0.2, 0.04],   type=float, help='force convergence limit 0.01, [force coefficient 0.04]')
    ...
    ### if not MD, it's training/test
        ...
        amp_jobs(fname,args.mod,args.job,args.test_force,args.pot,args.load_pot,args.hidden_layer,args.e_conv,args.f_conv,args.ncore,args.mol_atoms,args.ndata_total,args.ndata_train,args.data_s_type,args.data_s_list,args.g,args.twinx,descriptor=des_obj)
    return
