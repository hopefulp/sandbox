...
def amp_jobs(fdata, job, dstype, dslist, amp_pes, HL, E_conv, f_conv, Lgraph, ncore, n_mol, Ltwinx):
    total_images = ase.io.read(fdata, index=':')    # can read extxyz, OUTCAR, 
    images_sets = Images(total_images, nsets=dslist)
    ...
    ### JOB == TEST
        ...
        ### for data interval
            ...
            images = images_sets.get_test_images(d_list=dslist)
...
def main():
    ...
    parser.add_argument('-dt','--data_s_type',default='nset',choices=['nset','interval','sort','pick'], help='data selection type')
    ...
    amp_jobs(fname,args.job,args.data_s_type,args.data_s_list,args.pot,args.hidden_layer,args.e_conv,args.f_conv,args.g,args.ncore,args.nmol,args.twinx)
    return
