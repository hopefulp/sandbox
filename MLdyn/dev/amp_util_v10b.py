...
def stat_forces_filesum(yf3d, ybar_f3d, fpre=None):
    fname1 = "test_force_stat.dat"
    fname2 = "test_fmaxres"
    fname3 = "test_fstat_all.txt"
    pklfile = "test_fstat_all.pkl"
    f = open(fname1, 'w')

    ### variables for all file sum
    true_f = []
    hypo_f = []
    ### loop for files
    i_maxres_file = -1
    ...
    for i_image, (yf2d, ybar_f2d) in enumerate(zip(yf3d, ybar_f3d)):
        ...
        f_maxres        = np.max(np.absolute(yfbar_1d-yf_1d))
        f_maxres_ind    = np.argmax(np.absolute(yfbar_1d-yf_1d))
        ...
        ### if fpre: write force file for each image 
        if fpre:
    ...
    p = my_stat.stat_2col(yf_1d, yfbar_1d)
    fname2 += f"_img{i_maxres_file:03d}.dat"
    write_force_image(fname2, p, yf3d[i_file], ybar_f3d[i_file], tmp_maxres, i_maxres_ind)
    f.close()
...
### scan all test images: need to decompose into scan and write one image
def stat_forces_files(yf3d, ybar_f3d, fpre=None):
    fname1 = "test_forcefiles_stat.dat"
    fname2 = "test_fmaxres"
    f = open(fname1, 'w')
    ### loop for files
    i_maxres_file = -1
