...
def run_qchem_all(m_tag, patterns, Rem_file, isrun, file_type, pbs_long, ncore):
    ...
    if not os.getenv('HOST'):
        print("check hostname with 'HOST'")
    else:
        host = os.getenv('HOST')
    if host == "chi":
        version="serial"
    ...
    # otherwise get files by patterns
        ...
        l_file = get_files_pattern(m_tag, patterns, file_type, pwd)
    for fname in l_file:
        print(fname)
        lfile_name=re.split("\.",fname)
        if len(lfile_name) >= 3:
        ...
        if os.path.isfile(qchem_outf):
            #if File_list:
            print("As for %s, there exists .out file" % fname)
            continue
        else:
            nmol += 1
        ...
        if file_type == "mol" and nmol == 1 :
            if Rem_file:
                ...
                if len(rem_files) == 1:
                    ...
                    #print rem_files
                    if re.search('irc', fname, re.IGNORECASE):
                        Rfiles = [ s for s in rem_files if 'irc' in s]   # [   ] returns list
                    elif re.search('ts', fname, re.IGNORECASE):
...
def main():
    ...
    group.add_argument('-i', '--infile',  nargs='+', help='input input-files explicitly')
    ...
    group.add_argument('-a', '--allfile', action='store_true', help="run for all the files")
    parser.add_argument('-t', '--ftype', type=str, default='mol', choices=['mol', 'in'])
    ...
    run_qchem_all(m_tag, matching, args.rem, args.exe, args.ftype, args.pbs_long, args.core)
    return    
