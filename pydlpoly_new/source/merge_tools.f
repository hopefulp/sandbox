...
c     systolic data pulse to transfer data
     ...
     x        MPI_ANY_SOURCE,Merge_tag+k,pydlpoly_comm,request,ierr)

         call MPI_SEND(buffer(1),3*nsize,MPI_DOUBLE_PRECISION,jdnode,
     x        Merge_tag+k,pydlpoly_comm,ierr)
...
cRS   copy of merge .. also merges integer image arrays if we want to track
c     the particles e.g. for polarization calculations
      subroutine merge_track(idnode,mxnode,natms,nbuff,nibuff,
     x                        xxx,yyy,zzz,buffer,
     x                        ixx,iyy,izz,ibuffer)
...
c*********************************************************************
      ...
      real*8 xxx(natms),yyy(natms),zzz(natms),buffer(nbuff)
      integer nibuff
      integer ixx(natms),iyy(natms),izz(natms),ibuffer(nibuff)
...
c     check that buffer is large enough
      ...
      nsize=(natms+mxnode-1)/mxnode
      if(nbuff.lt.6*nsize)call error(idnode,47)
cRS test intgere buffer too
      if(nibuff.lt.6*nsize)call error(idnode,47)
...
c     systolic data pulse to transfer data
     ...
     x        Merge_tag+k,pydlpoly_comm,ierr)
         ...
         call MPI_IRECV(ibuffer(3*nsize+1),3*nsize,MPI_INTEGER,
     x        MPI_ANY_SOURCE,Merge_tag+k,pydlpoly_comm,request,ierr)

         call MPI_SEND(ibuffer(1),3*nsize,MPI_INTEGER,jdnode,
     x        Merge_tag+k,pydlpoly_comm,ierr)
...
c     out going message size
     ...
     x    MPI_ANY_SOURCE,Merge1_tag+k,pydlpoly_comm,request,ierr)
        
        call MPI_SEND(nout,1,MPI_INTEGER,jdnode,
     x    Merge1_tag+k,pydlpoly_comm,ierr)
...
c	identity of incoming groups 
     ...
     x        MPI_ANY_SOURCE,Merge4_tag+k,pydlpoly_comm,request,ierr)

         call MPI_SEND(buffer(1),4*nsize,MPI_DOUBLE_PRECISION,jdnode,
     x        Merge4_tag+k,pydlpoly_comm,ierr)
...
c     systolic data pulse to transfer data
     ...
     x    MPI_ANY_SOURCE,Shell_tag+k,pydlpoly_comm,request,ierr)

        call MPI_SEND(buffer(1),nsize,MPI_DOUBLE_PRECISION,jdnode,
     x    Shell_tag+k,pydlpoly_comm,ierr)
...
c     check for zero length messages
     ...
     x        MPI_ANY_SOURCE,Shmove_tag+k,pydlpoly_comm,request,ierr)

         if(i.gt.0) call MPI_SEND(buffer(1),i,MPI_DOUBLE_PRECISION,
     x        jdnode,Shmove_tag+k,pydlpoly_comm,ierr)
