...
cRS haaa!! this took me a while on 64 bit integer is not good
cRS        and pointer does not exist for gfortran
...
ccs
      if(lgauss)then
        allocate(ft_pot(mxatms),stat=fail(8))
      endif
ccs
...
CFFT      call dlpfft3(1,1,kmax1,kmax2,kmax3,key1,key2,key3,
CFFT     x  ww1,ww2,ww3,qqq)
...
c     calculate inverse 3D FFT of charge array (in place).
      ...
      call dfftw_execute_dft(fplan,qqq,qqq)
c$$$      call fftwnd_f77_one(fplan,qqq,0)
CESSL      inc2=kmaxd
...
CFFT      call dlpfft3(0,1,kmax1,kmax2,kmax3,key1,key2,key3,
CFFT     x  ww1,ww2,ww3,qqq)
...
c     calculate convolution of charge array with gaussian function
      ...
      call dfftw_execute_dft(bplan,qqq,qqq)
c$$$      call fftwnd_f77_one(bplan,qqq,0)
CESSL      call dcft3(qqq,inc2,inc3,qqq,inc2,inc3,kmax1,kmax2,kmax3,
CESSL     x  1,1.d0,buffer,mxbuff)
...
ccs
      if(lgauss)then
        do i=1,natms
          pot(i) = pot(i) + (ft_pot(i)*den)
c     store self interaction potential for each j
          pot(i) = pot(i)
     x       - 2.0d0*r4pie0/epsq * chge(i) * alpha/sqrpi
c     store dipole correction potential as obtained from qfix, see line 613
          pot(i) = pot(i)
     x         - (pi/volm*r4pie0/epsq)*(qchg/(alpha**2))
        enddo
        pot=pot/(dble(mxnode))
      endif
ccs
...
ccs   calculate product dQ/dq_i * (Phi_rec * Q)
              if(lgauss)then
                act_pot = -fac*qsum*bspx(i,j)*bspy(i,k)*bspz(i,l)
                ft_pot(i) = ft_pot(i) + act_pot
              endif
ccs
...
ccs
      if(mxnode.gt.1.and.lgauss) call gdsum(ft_pot,natms,buffer)
ccs
