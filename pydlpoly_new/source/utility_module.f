...
CVAM
      ...
      subroutine images_track
     x  (imcon,idnode,mxnode,natms,cell,xxx,yyy,zzz,imgx,imgy,imgz)
...
c
c     RS  pydlpoly
c     this is a copy of images above, which is called if ltrackimg is true
c     we additionally keep an integer rep of the image (which is zero initially)
c     
c***********************************************************************
...
c        write (*,*) "DEBUG before ", zzz(22), zzz(23), zzz(24)
        ...
        do i=iatm1,iatm2
          ...
          imgy(i)=imgy(i)+idy
          imgz(i)=imgz(i)+idz
          xxx(i)=xxx(i)-cell(1)*idx
          yyy(i)=yyy(i)-cell(1)*idy
          zzz(i)=zzz(i)-cell(1)*idz
        enddo

c        write (*,*) "DEBUG after  ", zzz(22), zzz(23), zzz(24)
...
c     rectangular (slab) boundary conditions
        ...
        do i=iatm1,iatm2
          ...
          imgz(i)=imgz(i)+idz          
          xxx(i)=xxx(i)-cell(1)*idx
          yyy(i)=yyy(i)-cell(5)*idy
          zzz(i)=zzz(i)-cell(9)*idz
...
c     parallelepiped boundary conditions
        ...
        do i=iatm1,iatm2
          ...
          imgx(i)=imgx(i)+nint(ssx)
          imgy(i)=imgy(i)+nint(ssy)
          imgz(i)=imgz(i)+nint(ssz)
...
cRS replace by standard F90 random generator, can be seeded to have "true" randomness
cRS   the original function gives always the same answer (this works parallel)
cRS   we broadcast instead 
cRS   NOTE: it might be better to braodcast e.g. the velocity array in gauss below
cRS         if duni is not used anywhere else
      function duni()
        use comm_module
        implicit none      
        real(8) duni
        ...
        call random_number(duni)
        call MPI_bcast(duni,1,MPI_DOUBLE_PRECISION,
...
c c*********************************************************************
c c     
c c     dl_poly random number generator based on the universal
c c     random number generator of marsaglia, zaman and tsang
c c     (stats and prob. lett. 8 (1990) 35-39.) it must be
c c     called once to initialise parameters u,c,cd,cm
c c     
c c     copyright daresbury laboratory 1992
c c     author -  w.smith         july 1992
c c     
c c     wl
c c     2008/12/23 10:29:12
c c     1.9
c c     Exp
c c     
c c*********************************************************************
...
c 
c       logical new
c       integer ir,jr,i,j,k,l,m,ii,jj
c       real(4) s,t,u,c,cd,cm,uni
c       real(8) duni
c       dimension u(97)
c       save u,c,cd,cm,uni,ir,jr,new
c       data new/.true./
c 
c CVAM
c CVAM      call VTBEGIN(134, ierr)
c CVAM
c 
c       if(new)then
c 
c c     initial values of i,j,k must be in range 1 to 178 (not all 1)
c c     initial value of l must be in range 0 to 168.
c 
c         i=12
c         j=34
c         k=56
...
c           u(ii)=s
c   200   continue
c         c =  362436.0/16777216.0
c         cd= 7654321.0/16777216.0
c         cm=16777213.0/16777216.0
c       else
c 
c c     calculate random number
c         uni=u(ir)-u(jr)
c         if(uni.lt.0.0)uni=uni+1.0
c         u(ir)=uni
c         ir=ir-1
c         if(ir.eq.0)ir=97
c         jr=jr-1
c         if(jr.eq.0)jr=97
c         c=c-cd
c         if(c.lt.0.0)c=c+cm
c         uni=uni-c
c         if(uni.lt.0.0)uni=uni+1.0
c         duni=dble(uni)
c       endif
c CVAM
c CVAM      call VTEND(134, ierr)
c CVAM
c       return
c       end function duni
