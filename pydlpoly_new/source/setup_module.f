...
cRS   NOTE: this value is soemwhat wrong (probably an old epsilon_0 value?)
cRS         and it differs from other values in other codes 
...
c      integer, parameter :: nrite=60
      integer             :: nrite=60

ccs   timings file output channel
...
ccs   max number of sigma values, equal to max number of atoms
      integer maxsgm
ccs   true if gaussian charges are activated
      logical lgauss, qeq_iter, calc_hess, all_coul, qeq_coulonly
ccs
crs   flags for sp-basis
      logical lspbasis
      save lspbasis
...
ccs
      save  maxsgm,lgauss,qeq_iter,calc_hess,all_coul,qeq_coulonly
      data qeq_iter,calc_hess,all_coul,qeq_coulonly
...
c     prescreen CONTROL to find out about charge representation
      call conscan_pre(idnode)
...
ccs   set max number of Gaussians to max number of sites

      if(lgauss) maxsgm = mxatms
...
cRS   BUGFIX ... need 4params for cos4 .. plus scaling vdw and coul we need 6 total (original 5)

      mxpdih=6
      
c     maximum number of parameters for inversion potentials
cJPD  for quartic increase from 2 to 4
...
c      write (*,*) "DEBUG: this is SETUP_MODULE"
c      write (*,*) "DEBUG: determining mxlist (ratio, dens)",ratio,dens
c      write (*,*) "DEBUG: current mxlist = ", mxlist
...
c      write (*,*) "DEBUG: after loglnk test, mxlist = ", mxlist
      ...
      mxlist=max(1000,mxlist)

c      write (*,*) "DEBUG: final mxlist = ", mxlist
              ...
              elseif(findstring('atoms',record,idum))then
                ...
                do isite=1,numsit
                  if(ksite.lt.numsit)then
                    ...
                    ppp=dblstr(record,lenrec,idum)
cRS added this wrt to the orig dl_poly (replaces chris construction to detect a "."!!)
c    by using conscan_pre to determine if we have gaussians and sp-basis
                    if(lgauss)then
c                     read another double (sigma)
                      ppp=dblstr(record,lenrec,idum)
                      if(lspbasis)then
c                        read another double : sigma of p (note that p-charges can not be read in and are assumed to be zero always)
                         mxspsites=mxspsites+1
                         ppp=dblstr(record,lenrec,idum)
                         if (ppp.ne.0.0d0) mxspsites=mxspsites+3
                      endif
                    endif
...
c     upper limit of 512
          ...
          elseif(findstring('free',record,idum))then
            ...
            lfree=.true.
ccs
          elseif(findstring('gaussi',record,idum))then

            lgauss=.true.

          elseif(findstring('all cterms',record,idum))then

            all_coul=.true.
ccs
          elseif(findstring('sp-basis',record,idum))then
...
cRS adding a routine conscan_pre 
c     this is a prescan of the CONTROL file to determine if we use gaussians (means there is a sigma after the charge in the FIELD)
c     and if we use sp-basis in addition (there are two charge/sigma pairs)
c     the reason for that is that the FIELD file is scanned (fldscan) BEFORE the CONTROL is scanned.
c     since we allow the FIELD format to change by the CONTROL this is necessary

      subroutine conscan_pre
     x (idnode)
      ...
      do while(peek)
        ...
        if(record(1).ne.'#')then
          ...
          if(findstring('gaussi',record,idum))then

            lgauss=.true.
c            write (*,*)  "DEBUG DEBUG Gaussians used"

            
          elseif(findstring('sp-basis',record,idum))then
            
            lspbasis=.true.
c            write (*,*)  "DEBUG DEBUG sp-basis used"
