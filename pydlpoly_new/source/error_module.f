...
c#######################################################################
      ...
      if(idnode.eq.0)then
        ...
        if(kill)then
          write (*,'(/,/,a)') 
     x      'Error: See manual for Details on Error Codes!' 
          write(6,'(/,/,1x,a,i5)') 
     x      'PYDLPOLY terminated due to error ', kode

        else

          write(6,'(/,/,1x,a,i5)') 
     x      'PYDLPOLY will terminate due to error ', kode
...
c     dummy entry
     ...
     x        'error - cannot run both TAD and BPD together'
          ...
          elseif(kode.eq.2360)then
            write(6,'(/,/,1x,a)')
     x        'error - Wrong switch value (must be between 0.0 and 1.0)'
